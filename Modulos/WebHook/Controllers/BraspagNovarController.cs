...
{
	#region [ BraspagNovarController ]
	public class BraspagNovarController : ApiController
	{
		#region [ GetIp ]
		...
		{
			#region [ Declarações ]
			const string NOME_DESTA_ROTINA = "BraspagNovarController.Teste()";
		...
		{
			#region [ Declarações ]
			const string NOME_DESTA_ROTINA = "BraspagNovarController.Post()";
			string strLogAtividade = NOME_DESTA_ROTINA + " (origem: " + (GetIp() ?? "") + ")";
			...
			try // Try-Catch-Finally
			{
				...
				try
				{
					intLinhasAfetadas = BD.Grava(parameters, empresa: "NOVAR");
