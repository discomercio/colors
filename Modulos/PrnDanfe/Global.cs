...
namespace PrnDANFE
{
	class Global
	{
		...
		public class Cte
		{
			...
			/*================================================================================================
			 * v 1.00 - 06.05.2014 - por LHGX
			 *        Início.
			 *        Este programa realiza a impressão em lote dos PDF's de DANFE que foram marcados p/
			 *        impressão no Relatório de Solicitação de Coletas (Pedidos Prontos para Romaneio).
			 * -----------------------------------------------------------------------------------------------
			 * v 1.01 - 13.05.2014 - por LHGX
			 *		  - alteração do nome da pasta de trabalho de PDFs (de ManipulaPDF para _tempWorkingDir)
			 *		  - salvamento simultâneo de arquivos agrupados e arquivos individuais, para qualquer 
             *		    operação solicitada (gravação ou impressão)
			 * -----------------------------------------------------------------------------------------------
			 * v 1.02 - 17.05.2014 - por LHGX
			 *		  - substituição do botão "Gravar DANFEs selecionadas em um único arquivo" pelo botão 
			 *		    "Abrir a pasta de PDFs agrupados"
			 *		  - substituição do botão "Gravar DANFEs selecionadas em um arquivos separados" pelo botão 
			 *		    "Abrir a pasta de PDFs individuais"
			 * -----------------------------------------------------------------------------------------------
			 * v 1.03 - 23.01.2015 - por HHO
			...
			public class Etc
			{
				...
				public static string PathManipulaPDF = Application.StartupPath + "\\_tempWorkingDir";
				public static string ArqMergePDF = PathManipulaPDF + "\\MergePDF.pdf";
				public static string PathPDFAgrupado = Application.StartupPath + "\\PDF_AGRUPADO";
				public static string PathPDFIndividual = Application.StartupPath + "\\PDF_INDIVIDUAL";
			...
			#region [ T_PEDIDO__ST_ETG_IMEDIATA ]
			public class T_PEDIDO__ST_ETG_IMEDIATA
			{
				public const int ST_INICIAL = 0;
				public const int OPCAO_NAO = 1;
				public const int OPCAO_SIM = 2;
				public const int NAO_DEFINIDO = 10;
			}
			#endregion

			#region [ Classe PrnDanfe ]
			public class PrnDanfe
			{
				#region [ LogOperacao - Códigos de operação para o log ]
				public class LogOperacao
				{
					// Texto com 20 posições
					public const String LOGON = "PrnDanfe-Logon";
					public const String LOGOFF = "PrnDanfe-Logoff";
					public const String PRNDANFE_GRAVACAO_CONCATENA = "PrnDanfe-Gravar-Concatenado";
					public const String PRNDANFE_GRAVACAO_SEPARA = "PrnDanfe-Gravar-Separado";
					public const String PRNDANFE_IMPRESSAO = "PrnDanfe-Imprimir";
					public const String RECONEXAO_BD = "PrnDanfe-Reconexao-BD";
				}
				#endregion
			}
			#endregion
		...
		{
			#region [ Constantes ]
            public const String OP_CEN_PEDIDO_EXIBIR_LINK_DANFE = "23000";
			#endregion
		...
		public class Usuario
		{
			...
			public static String strPastaEmitente = "";
		...
		#region [ formataHhMmSsComSigla ]
		public static String formataHhMmSsComSigla(DateTime dataHora)
		{
			#region [ Declarações ]
			string[] v;
			string strResp;
			...
			v = dataHora.ToString(Global.Cte.DataHora.FmtHhMmSsComSeparador).Split(':');
			if (v.Length < 3) return "";
			strResp = v[0] + "h" + v[1] + "m" + v[2] + "s";
			return strResp;
		}
		#endregion
