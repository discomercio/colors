...
namespace PrnDANFE
{
	public partial class FMain : PrnDANFE.FModelo
	{
		...
		public FMain()
		{
			...
			if (!Directory.Exists(Global.Cte.Etc.PathManipulaPDF)) Directory.CreateDirectory(Global.Cte.Etc.PathManipulaPDF);
			if (!Directory.Exists(Global.Cte.Etc.PathPDFAgrupado)) Directory.CreateDirectory(Global.Cte.Etc.PathPDFAgrupado);
			if (!Directory.Exists(Global.Cte.Etc.PathPDFIndividual)) Directory.CreateDirectory(Global.Cte.Etc.PathPDFIndividual);
		}
		#endregion
		...
		#region [ trataBotaoImprimirDANFEPDF ]
		private void trataBotaoImprimirDANFEPDF()
		{
			...
			fDANFEImprime = new FDANFEImprime();
			fDANFEImprime.Location = this.Location;
			fDANFEImprime.Show();
			if (!fDANFEImprime.ocorreuExceptionNaInicializacao) this.Visible = false;
		}
		#endregion
		...
		public static bool reiniciaBancoDados()
		{
			...
			try
			{
				...
				log.operacao = Global.Cte.PrnDanfe.LogOperacao.RECONEXAO_BD;
		...
		private void FMain_Shown(object sender, EventArgs e)
		{
			...
			try
			{
				...
				if (!_InicializacaoOk)
				{
					...
					do
					{
						...
						try
						{
							...
							if (blnValidacaoUsuarioOk)
							{
								...
								if (!Global.Acesso.operacaoPermitida(Global.Acesso.OP_CEN_PEDIDO_EXIBIR_LINK_DANFE))
								{
					...
					#region [ Seleciona Centro de Distribuição ]
					...
					if (BD.obtem_emitentes_usuario(Global.Usuario.usuario, ref Global.Usuario.listaEmitentes, ref qtdEmits, ref strMsgErro))
					{
						if (qtdEmits == 1)
						{
							...
							Global.Usuario.strPastaEmitente = Global.Usuario.listaEmitentes[0].emitente_nome;
						}
						else
						{
					...
					versaoModulo = BD.getVersaoModulo("PrnDanfe", out strMsgErro);
					...
					log.usuario = Global.Usuario.usuario;
					log.operacao = Global.Cte.PrnDanfe.LogOperacao.LOGON;
					...
					if (!PDF.testaAcrobat())
					{
						aviso("Adobe Acrobat não detectado neste computador!\nO programa pode não funcionar corretamente!");
					}
		...
		private void FMain_FormClosing(object sender, FormClosingEventArgs e)
		{
			...
			if (_InicializacaoOk)
			{
				...
				log = new Log();
				log.usuario = Global.Usuario.usuario;
				log.operacao = Global.Cte.PrnDanfe.LogOperacao.LOGOFF;
		...
		#region [ btnImprimirDANFEPDF ]

		#region [ btnImprimirDANFEPDFs_Click ]
		private void btnImprimirDANFEPDF_Click(object sender, EventArgs e)
		{
			trataBotaoImprimirDANFEPDF();
		}
		#endregion
