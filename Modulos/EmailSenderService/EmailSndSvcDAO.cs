...
namespace EmailSenderService
{
	class EmailSndSvcDAO
	{
		...
		public const int USAR_REPLYTOMSG_NAO = 0;
		public const int USAR_REPLYTOMSG_SIM = 1;
		#endregion
		...
		public static void inicializaObjetosEstaticos()
		{
			...
			cmInsereMensagem.Parameters.Add("@replyToMsg", SqlDbType.VarChar, 1024);
			cmInsereMensagem.Parameters.Add("@st_replyToMsg", SqlDbType.SmallInt);
												...
												out String msg_erro_grava_rem)
		{
			...
			try
			{
				...
				try
				{
					...
					if (!BD.geraNsu(EssNsu.T_EMAILSNDSVC_REMETENTE, out id_remetente, out msg_erro_grava_rem))
					{
		...
		/// </param>
		/// <param name="replyToMsg">
		/// Um ou mais e-mails que receberão a resposta da mensagem, se houver. Os e-mails podem ser separados por espaço em branco,
		/// vírgula ou ponto e vírgula.
		/// <param name="st_replyToMsg">
		/// Campo que indica se o campo replyTo da mensagem será preenchido pelo campo anterior (replyToMsg) ou não
		/// 0 = Não / 1 = Sim
													...
													out String msg_erro_grava_msg)
		{
			...
			try
			{
				...
				//verifica se o campo replyToMsg está preenchido com caracteres válidos
				if (replyToMsg.Trim() != "")
				{
					if (!isEmailValido(replyToMsg, ref strEmailListaErros))
					{
						msg_erro_grava_msg = "E-mail do campo <<replyToMsg>> preenchido com caracteres inválidos";
						return false;
					}
					else
					{
						if (strEmailListaErros.Trim() != "")
						{
							msg_erro_grava_msg = "Há e-mail  do campo <<replyToMsg>> preenchido com caracteres inválidos: " + strEmailListaErros;
							return false;
						}
					}
				}
				//verificar se a definição da origem do campo replyTo está preenchido corretamente
				if ((st_replyToMsg != EmailSndSvcDAO.USAR_REPLYTOMSG_NAO) && (st_replyToMsg != EmailSndSvcDAO.USAR_REPLYTOMSG_SIM))
				{
					msg_erro_grava_msg = "Problema no conteúdo do campo <<st_replyMsg>>";
					return false;
				}
				...
				try
				{
					...
					if (!BD.geraNsu(EssNsu.T_EMAILSNDSVC_MENSAGEM, out id_mensagem, out msg_erro_grava_msg))
					{
					...
					if (replyToMsg.Trim() == "")
					{
						cmInsereMensagem.Parameters["@replyToMsg"].Value = DBNull.Value;
					}
					else
					{
						cmInsereMensagem.Parameters["@replyToMsg"].Value = replyToMsg;
					}
					cmInsereMensagem.Parameters["@st_replyToMsg"].Value = st_replyToMsg;
					#endregion
