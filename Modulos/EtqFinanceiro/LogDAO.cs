...
namespace EtqFinanceiro
{
    ...
    {
        #region [ Atributos ]
        private static SqlCommand _cmdInsereLog;
        #endregion
        ...
        static LogDAO()
        {
            ...
            #region [ Prepara Sql e parâmetros ]
            strSql = "INSERT INTO t_LOG (" +
                    ...
                    ") VALUES (" +
                        ...
                        "@complemento" +
                    ")";
            _cmdInsereLog = BD.criaSqlCommand();
            _cmdInsereLog.CommandText = strSql;
            _cmdInsereLog.Parameters.Add("@usuario", SqlDbType.VarChar, 10);
            _cmdInsereLog.Parameters.Add("@loja", SqlDbType.VarChar, 3);
            _cmdInsereLog.Parameters.Add("@pedido", SqlDbType.VarChar, 9);
            _cmdInsereLog.Parameters.Add("@id_cliente", SqlDbType.VarChar, 12);
            _cmdInsereLog.Parameters.Add("@operacao", SqlDbType.VarChar, 20);
            _cmdInsereLog.Parameters.Add("@complemento", SqlDbType.Text, -1);
            _cmdInsereLog.Prepare();
            #endregion
        }
        #endregion
                                ...
                                )
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    _cmdInsereLog.Parameters["@usuario"].Value = log.usuario;
                    _cmdInsereLog.Parameters["@loja"].Value = (log.loja == null ? "" : log.loja);
                    _cmdInsereLog.Parameters["@pedido"].Value = (log.pedido == null ? "" : log.pedido);
                    _cmdInsereLog.Parameters["@id_cliente"].Value = (log.id_cliente == null ? "" : log.id_cliente);
                    _cmdInsereLog.Parameters["@operacao"].Value = (log.operacao == null ? "" : log.operacao);
                    _cmdInsereLog.Parameters["@complemento"].Value = (log.complemento == null ? "" : log.complemento);
                    #endregion

                    #region [ Monta texto para o log em arquivo ]
                    // Se houver conteúdo de alguma tentativa anterior, descarta
                    sbLog = new StringBuilder("");
                    foreach (SqlParameter item in _cmdInsereLog.Parameters)
                    {
                    ...
                    try
                    {
                        intRetorno = BD.executaNonQuery(ref _cmdInsereLog);
                    }
                    catch (Exception ex)
                    {
                        intRetorno = 0;
