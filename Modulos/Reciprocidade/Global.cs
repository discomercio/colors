	...
	class Global
	{
		...
		public class Cte
		{
			...
			public class Aplicativo
			{
				...
				public const string NOME_SISTEMA = "SerasaRecipr";
				public const string VERSAO_NUMERO = "1.07";
				public const string VERSAO_DATA = "27.FEV.2017";
				public const string VERSAO = VERSAO_NUMERO + " - " + VERSAO_DATA;
				public const string M_ID = NOME_SISTEMA + "  -  " + VERSAO;
				public const string M_DESCRICAO = "Módulo SerasaRecipr";
			}
			#endregion

			#region[ Comentário sobre as versões ]
			/*================================================================================================
			 * v 1.00 - 01.06.2014 - por TTT
			 *		  Início.
			 * -----------------------------------------------------------------------------------------------
			 * v 1.01 - 25.06.2014 - por HHO
			 *		  Ajustes em decorrência de novas informações recebidas durante o processo de
			 *		  homologação.
			 * -----------------------------------------------------------------------------------------------
			 * v 1.02 - 27.06.2014 - por HHO
			 *		  Ajustes em decorrência de novas informações recebidas durante o processo de
			 *		  homologação e do processo de testes/depuração.
			 *		  Alteração da periodicidade de "semanal" para "diário".
			 * -----------------------------------------------------------------------------------------------
			 * v 1.03 - 10.10.2014 - por TTT/HHO
			 *		  Implementação de relatório (em Excel) com a relação de títulos informados no arquivo
			 *		  de conciliação e que precisam de checagem manual porque há indícios de que foram
			 *		  quitados/baixados em nosso sistema e ainda constam como em atraso na Serasa.
			 *		  Implementada mensagem de progresso durante o processamento em várias telas.
			 * -----------------------------------------------------------------------------------------------
			 * v 1.04 - 29.10.2014 - por TTT/HHO
			 *		  Inclusão de nova coluna na planilha de checagem de títulos da conciliação com o campo
			 *		  ID do registro.
			 *		  Implementação de recurso p/ localizar um determinado registro pelo ID no grid de
			 *		  tratamento de títulos da conciliação.
			 *		  Ajustes na memorização dos paths dos arquivos de remessa e retorno.
			 * -----------------------------------------------------------------------------------------------
			 * v 1.05 - 23.JAN.2015 - por HHO
			 *		  Alteração nos dados de conexão ao BD devido à migração do servidor, pois o SQL Server
			 *		  não está mais usando a porta padrão por questões de segurança.
			 * -----------------------------------------------------------------------------------------------
			 * v 1.06 - 01.06.2016 - por HHO
			 ...
			 * -----------------------------------------------------------------------------------------------
			 * v 1.07 - 27.02.2017 - por HHO
			 *		  Implementação de ajustes para operar a Reciprocidade Serasa com a DIS: alteração
			 *		  da constante 'CNPJ_EMPRESA_CONVENIADA' por variável cujo valor é lido do arquivo de
			 *		  configuração.
			 * -----------------------------------------------------------------------------------------------
			...
			#region [ SerasaReciprocidade ]
			public class SerasaReciprocidade
			{
				public static readonly string CNPJ_EMPRESA_CONVENIADA = Global.digitos(Global.GetConfigurationValue("CnpjEmpresaConveniada") ?? "");

				// IMPORTANTE: PARA ALTERAR A PERIODICIDADE, É NECESSÁRIO SOLICITAR P/ A SERASA ALTERAR A CONFIGURAÇÃO NO SISTEMA DELES TAMBÉM!!
				//public const string CODIGO_PERIODICIDADE_ARQ_REMESSA = "D";
				//public const int TEMPO_PERIODICIDADE_ARQ_REMESSA_EM_DIAS = 1;

				public const string CODIGO_PERIODICIDADE_ARQ_REMESSA = "S";
				public const int TEMPO_PERIODICIDADE_ARQ_REMESSA_EM_DIAS = 7;
			}
			#endregion
			...
			#region [ Classe Reciprocidade ]
			public class Reciprocidade
			{
				#region [ LogOperacao - Códigos de operação para o log ]
				public class LogOperacao
				{
					// Texto com 20 posições
					public const String LOGON = "Reciprocidade-Logon";
					public const String LOGOFF = "Reciprocidade-Logoff";
				}
				#endregion
			}
			#endregion

			#region [ Tabelas Serasa ]

			public static string t_SERASA_CLIENTE = "t_SERASA_CLIENTE";
			public static string t_SERASA_TITULO_MOVIMENTO = "t_SERASA_TITULO_MOVIMENTO";
			public static string t_SERASA_ARQ_REMESSA_NORMAL = "t_SERASA_ARQ_REMESSA_NORMAL";
			public static string t_SERASA_REMESSA_DET_TEMPO_RELAC = "t_SERASA_REMESSA_DET_TEMPO_RELAC";
			public static string t_SERASA_REMESSA_DET_TITULO = "t_SERASA_REMESSA_DET_TITULO";
			public static string t_SERASA_ARQ_RETORNO_NORMAL = "t_SERASA_ARQ_RETORNO_NORMAL";
			public static string t_SERASA_RETORNO_NORMAL_TITULO = "t_SERASA_RETORNO_NORMAL_TITULO";
			public static string t_SERASA_RETORNO_NORMAL_TAB_ERROS = "t_SERASA_RETORNO_NORMAL_TAB_ERROS";
			public static string t_SERASA_ARQ_CONCILIACAO_OUTPUT = "t_SERASA_ARQ_CONCILIACAO_OUTPUT";
			public static string t_SERASA_ARQ_CONCILIACAO_INPUT = "t_SERASA_ARQ_CONCILIACAO_INPUT";
			public static string t_SERASA_CONCILIACAO_TITULO = "t_SERASA_CONCILIACAO_TITULO";
		...
		public static string PATH_DEFAULT_TITULO_ARQUIVO_REMESSA = Application.StartupPath + "\\TITULOS\\ARQUIVO_REMESSA";
		...
		{
			#region [ Constantes ]
			public const String OP_CEN_SERASA_RECIPROCIDADE_ACESSO_AO_MODULO = "26000";
			#endregion
		...
		public class Usuario
		{
			...
			public class Defaults
			{
				...
				public class FArqRemessa
				{
					public static String pathTituloArquivoRemessa = "";

				}
				public class FArqRetorno
				{
					public static String pathTituloArquivoRetorno = "";
				}
				public class FArqRetornoConciliacao
				{
					public static String pathTituloArquivoRetornoConciliacao = "";
				}
			}
			#endregion
		}
		...
		public class RegistryApp
		{
			...
			public class Chaves
			{
				...
				public class FArqRetorno
				{
					public static String pathTituloArquivoRetorno = "pathTituloArquivoRetorno";
				}
				public class FArqRetornoConciliacao
				{
					public static String pathTituloArquivoRetornoConciliacao = "pathTituloArquivoRetornoConciliacao";
				}
				public class FArqRemessa
				{
					public static String pathTituloArquivoRemessa = "pathTituloArquivoRemessa";
				}
			}
			#endregion
		...
		#region [ formataDataYyyyMmDdSemSeparador ]
		public static String formataDataYyyyMmDdSemSeparador(DateTime d)
		{
			StringBuilder ano = new StringBuilder(); ano.Append(d.Year);
			StringBuilder mes = new StringBuilder(); mes.Append(d.Month);
			if (mes.Length == 1) mes.Insert(0, '0');

			StringBuilder dia = new StringBuilder(); dia.Append(d.Day);
			if (dia.Length == 1) dia.Insert(0, '0');

			StringBuilder data = new StringBuilder();
			data.Append(ano).Append(mes).Append(dia);

			return data.ToString();
		}
		#endregion

		#region [ formataMoedaSemSeparador ]
		public static String formataMoedaSemSeparador(decimal valor, int tamanho)
		{
			String valorSemSeparador = formataMoeda(valor).Replace(",", "").Replace(".", "");
			StringBuilder sbValor = new StringBuilder(valorSemSeparador);
			int currentSize = sbValor.Length;
			sbValor.Insert(0, "0", tamanho - currentSize);
		...
		#region [ converteMoedaSemSeparadorParaDecimal ]
		public static decimal converteMoedaSemSeparadorParaDecimal(String valor)
		{
			valor = valor.Trim().Insert(valor.Length - 2, ",");
			return converteNumeroDecimal(valor);
		}
		#endregion

		#region [ converteYyyyMmDdSemSeparadorParaDateTime ]
		public static DateTime converteYyyyMmDdSemSeparadorParaDateTime(String dataFormatoSerasa)
		{
			if (dataFormatoSerasa.Trim().Length != 8)
			{
				return DateTime.MinValue;
			}
			...
			int ano = Convert.ToInt32(dataFormatoSerasa.Substring(0, 4).Trim());
			int mes = Convert.ToInt32(dataFormatoSerasa.Substring(4, 2).Trim());
			int dia = Convert.ToInt32(dataFormatoSerasa.Substring(6, 2).Trim());

			if (dia < 1 || dia > 31) blnErro = true;
			if (mes < 1 || mes > 12) blnErro = true;

			if (blnErro) return DateTime.MinValue;
