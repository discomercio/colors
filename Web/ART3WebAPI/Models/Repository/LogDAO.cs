...
namespace ART3WebAPI.Models.Repository
{
    class LogDAO
    {
        ...
        /// </returns>
        public static bool insere(Guid? httpRequestId, String usuario, Entities.Log log, out String strMsgErro)
        {
			...
			try // Finally: cn.Close()
			{
                ...
                try
                {
                    ...
                    if (blnSucesso)
                    {
                        Global.gravaLogAtividade(httpRequestId, NOME_DESTA_ROTINA + " - Log gravado com sucesso no BD:\n" + sbLog.ToString());
                        return true;
                    }
                    else
                    {
                        strMsgErro = "Falha ao gravar no banco de dados o log após " + intQtdeTentativas.ToString() + " tentativas!!";
                        Global.gravaLogAtividade(httpRequestId, NOME_DESTA_ROTINA + " - " + strMsgErro);
                        return false;
                    }
                    #endregion
                }
                catch (Exception ex)
                {
                    ...
                    Global.gravaLogAtividade(httpRequestId, NOME_DESTA_ROTINA + " - Exception: " + strMsgErro);
                    return false;
                }
            }
            finally
