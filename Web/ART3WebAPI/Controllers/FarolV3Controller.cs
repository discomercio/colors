    ...
    public class FarolV3Controller : ApiController
    {
        ...
        public async Task<HttpResponseMessage> GetXLSReport(string usuario, string dt_inicio, string dt_termino, string fabricante, string grupo, string subgrupo, string btu, string ciclo, string pos_mercado, string perc_est_cresc, string loja, string visao)
        {
            ...
            try
            {
                ...
                List<Farol> relFarolList = datasource.GetV3(dt_inicio, dt_termino, fabricante, grupo, subgrupo, btu, ciclo, pos_mercado, loja).ToList();

                if (relFarolList.Count != 0)
                {
                    await ART3WebAPI.Models.Domains.FarolGeradorRelatorio.GenerateXLSv3(relFarolList, filePath, dt_inicio, dt_termino, fabricante, grupo, subgrupo, btu, ciclo, pos_mercado, perc_est_cresc, loja, visao);
                    statusResponse = "OK";
