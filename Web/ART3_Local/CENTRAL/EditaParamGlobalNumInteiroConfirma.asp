...
<%
'     ===========================================================================
'	  E D I T A P A R A M G L O B A L N U M I N T E I R O C O N F I R M A . A S P
'     ===========================================================================
'
'
...
'	OBTÉM DADOS DO FORMULÁRIO ANTERIOR
	dim c_parametro, m_parametro
	c_parametro = retorna_so_digitos(Trim(Request.Form("c_parametro")))
	m_parametro = converte_numero(c_parametro)
	...
	alerta = ""
	if retorna_so_digitos(Cstr(m_parametro)) <> retorna_so_digitos(c_parametro) then
		alerta="VALOR DO PARÂMETRO DIGITADO COM FORMATO INVÁLIDO."
		end if
...
'	GRAVA PARÂMETRO!!
	if alerta = "" then 
		s = "SELECT * FROM t_CONTROLE WHERE (id_nsu = '" & strIdParametro & "')"
		r.Open s, cn
		if r.Eof then
			alerta = "PARAMETRO '" & strIdParametro & "' NÃO FOI ENCONTRADO EM T_CONTROLE."
			end if
		...
		if alerta = "" then
			...
			If Err = 0 then 
				...
				if s_log <> "" then 
					grava_log usuario, "", "", "", strIdParametro, s_log
					end if
			else
				erro_fatal=True
