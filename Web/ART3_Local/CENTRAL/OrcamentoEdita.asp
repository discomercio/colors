...
var blnConfirmaDifRAeValores=false;
	...
	if (s.length > MAX_TAM_OBS1) {
		alert('Conteúdo de "Observações I" excede em ' + (s.length-MAX_TAM_OBS1) + ' caracteres o tamanho máximo de ' + MAX_TAM_OBS1 + '!!');
		f.c_obs1.focus();
		return;
		}
...
//  Consiste a nova versão da forma de pagamento
	...
	//	Parcela Única
		...
		if (f.rb_forma_pagto[idx].checked) {
			...
			if (ve<vtFP) {
				alert('O valor do pagamento da parcela única está abaixo do valor do orçamento!!');
				f.c_pu_valor.focus();
				return;
				}
			if (blnConfirmaDifRAeValores) {
				if (ve>vtFP) {
					if (!window.confirm('O valor do pagamento da parcela única está ' + SIMBOLO_MONETARIO + ' ' + formata_moeda(ve-vtFP) + ' acima do valor nominal do orçamento.\nConfirma?')) {
						f.c_pu_valor.focus();
						return;
						}
					}
...
<% end if%>
		...
		if (blnTemEndEntrega) {
			...
			if (trim(f.EndEtg_endereco_numero.value)=="") {
				alert('Número do endereço de entrega não foi preenchido corretamente!!');
...
<% else %>
<!-- ****************************************************** -->
<!-- **********  PÁGINA PARA EDITAR O ORÇAMENTO  ********** -->
<!-- ****************************************************** -->
...
%>
	...
	<td width="145" class="MD" align="left"><p class="Rf">ORÇAMENTISTA</p><p class="C"><%=strTextoIndicador%>&nbsp;</p></td>
