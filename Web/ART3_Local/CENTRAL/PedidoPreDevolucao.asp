...
'	REVISADO P/ IE10
	...
	dim s, usuario, pedido_selecionado, id_devolucao, st_codigo, st_devolucao_descricao, st_devolucao_cor, st_usuario, st_data_hora
...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_CEN_PRE_DEVOLUCAO_LEITURA, s_lista_operacoes_permitidas) And _
        Not operacao_permitida(OP_CEN_PRE_DEVOLUCAO_ADMINISTRACAO, s_lista_operacoes_permitidas) then 
...
'	=========================
    ...
    r.Open s,cn
	if Not r.Eof then s_sessionToken = Trim("" & r("SessionTokenModuloCentral"))
    ...
    serverVariablesUrl = Mid(serverVariablesUrl, 1, CInt(InStr(serverVariablesUrl, "CENTRAL")-1))
    ...
    <tr>
        <td colspan="4">
            <table id="tblFotos" cellpadding="5" cellspacing="0">
                <tr>
<% if r.Eof then 
    if Not operacao_permitida(OP_CEN_PRE_DEVOLUCAO_ADMINISTRACAO, s_lista_operacoes_permitidas) Or st_codigo <> COD_ST_PEDIDO_DEVOLUCAO__CADASTRADA then
    %>
                    <td style="width: 100%;">
...
<% end if %>
                ...
                <% else %>
                    ...
                    <% if operacao_permitida(OP_CEN_PRE_DEVOLUCAO_ADMINISTRACAO, s_lista_operacoes_permitidas) And st_codigo = COD_ST_PEDIDO_DEVOLUCAO__CADASTRADA then %>
...
<% if i < PEDIDO_DEVOLUCAO_QTDE_FOTO And operacao_permitida(OP_CEN_PRE_DEVOLUCAO_ADMINISTRACAO, s_lista_operacoes_permitidas) And st_codigo = COD_ST_PEDIDO_DEVOLUCAO__CADASTRADA then 
...
<% end if %>
<% if operacao_permitida(OP_CEN_PRE_DEVOLUCAO_ADMINISTRACAO, s_lista_operacoes_permitidas) And st_codigo = COD_ST_PEDIDO_DEVOLUCAO__MERCADORIA_RECEBIDA then %>
<% if blnExibeBotaoConfirma then %>
</tr>
<tr>
<% end if %>
