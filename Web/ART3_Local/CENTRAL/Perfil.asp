...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if Not operacao_permitida(OP_CEN_CADASTRO_PERFIL, s_lista_operacoes_permitidas) then
...
var s_dest,s_op,s_perfil;
	...
	if (f.rb_op[0].checked) {
		s_dest="PerfilEdita.asp";
		s_op=OP_INCLUI;
		s_perfil=f.c_novo.value;
		if (trim(f.c_novo.value)=="") {
			alert("Forneça a identificação para o novo perfil!!");
			f.c_novo.focus();
			return false;
			}
		}
	...
	if (f.rb_op[1].checked) {
		s_dest="PerfilEdita.asp";
		s_op=OP_CONSULTA;
		s_perfil=f.c_cons.value;
		if (trim(f.c_cons.value)=="") {
			alert("Especifique o perfil a ser consultado!!");
			f.c_cons.focus();
			return false;
			}
		}
	...
	f.perfil_selecionado.value=s_perfil;
...
<span class="T">CADASTRO DE PERFIL</span>
