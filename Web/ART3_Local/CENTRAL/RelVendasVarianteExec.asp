...
<%
'     ======================================================
'	  R E L V E N D A S V A R I A N T E E X E C . A S P
'     ======================================================
'
'
...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_CEN_REL_VENDAS, s_lista_operacoes_permitidas) then 
		cn.Close
	...
	dim c_dt_inicio, c_dt_termino, c_loja, c_fabricante, c_produto, c_grupo, c_vendedor, c_indicador
	dim s_nome_vendedor
...
dim perc, vl_total_saida, vl_sub_total_saida
...
'	DE 'QTDE' DE T_PEDIDO_ITEM.
	...
	s_sql = "SELECT t_ESTOQUE_MOVIMENTO.fabricante AS fabricante, t_ESTOQUE_MOVIMENTO.produto AS produto," & _
			...
			s & _
			" GROUP BY t_ESTOQUE_MOVIMENTO.fabricante, t_ESTOQUE_MOVIMENTO.produto, t_PRODUTO.descricao, t_PRODUTO.descricao_html"
...
'	CRIADOS VÁRIOS REGISTROS DE ESTOQUE A DIFERENTES CUSTOS DE AQUISIÇÃO.
  ...
  ' CABEÇALHO
	...
	cab = "	<TR style='background:azure' NOWRAP>" & _
		  ...
		  "		<TD class='MTD' valign='bottom' NOWRAP><P style='width:240px' class='R'>Descrição</P></TD>" & _
		  ...
		  "		<TD class='MTD' align='right' valign='bottom'><P style='width:70px' class='Rd' style='font-weight:bold;'>% do Faturamento Total</P></TD>" & _
		  "	</TR>" & chr(13)
	...
	'	MUDOU DE FABRICANTE?
		  ...
		  ' FECHA TABELA DO FABRICANTE ANTERIOR
			if n_reg_total > 0 then 
				if vl_total_saida = 0 then
					...
					perc = (vl_sub_total_saida/vl_total_saida)*100
					end if
				s_cor="black"
			...
			s = s & s_aux
			x = x & cab_table
			if s <> "" then x = x & "<TR><TD class='MDTE' COLSPAN='5' valign='bottom' class='MB' style='background:azure;'><p class='N'>&nbsp;" & s & "</p></td></tr>" & chr(13)
			x = x & cab
			end if
	 ...
	 '>	PERCENTUAL DO FATURAMENTO TOTAL
		if vl_total_saida = 0 then
			...
			perc = (r("valor_saida")/vl_total_saida)*100
			end if
	...
	'>	TOTAL GERAL
		if qtde_fabricantes > 1 then
			if vl_total_saida = 0 then
				...
				perc = (vl_total_saida/vl_total_saida)*100
				end if
			s_cor="black"
			if qtde_total < 0 then s_cor="red"
			if vl_total_saida < 0 then s_cor="red"
			x = x & "<TR><TD COLSPAN='5' style='border-left:0px;border-right:0px;'>&nbsp;</td></tr>" & chr(13) & _
					"<TR><TD COLSPAN='5' style='border-left:0px;border-right:0px;'>&nbsp;</td></tr>" & chr(13) & _
  ...
  ' MOSTRA AVISO DE QUE NÃO HÁ DADOS!!
	if n_reg_total = 0 then
		...
		if c_fabricante <> "" then
			...
			if (s<>"") And (s_aux<>"") then s = s & " - "
			s = s & s_aux
			if s <> "" then x = x & cab_table & "<TR><TD class='MDTE' COLSPAN='5' valign='bottom' class='MB' style='background:azure;'><p class='N'>&nbsp;" & s & "</p></td></tr>" & chr(13) & cab
		else
			x = x & cab_table & cab
			end if

		x = x & "	<TR NOWRAP>" & _
				"		<TD class='MT' colspan='5'><P class='ALERTA'>&nbsp;NENHUM PRODUTO DO ESTOQUE SATISFAZ AS CONDIÇÕES ESPECIFICADAS&nbsp;</P></TD>" & _
				"	</TR>"
		end if
