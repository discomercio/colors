...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if Not operacao_permitida(OP_CEN_CADASTRO_FABRICANTES, s_lista_operacoes_permitidas) then 
...
var s_dest, s_op, s_fabricante;
	...
	if (f.rb_op[0].checked) {
		s_dest="fabricanteedita.asp";
		s_op=OP_INCLUI;
		s_fabricante=f.c_novo.value;
		if (trim(f.c_novo.value)=="") {
			alert("Forneça o número para o novo fabricante!!");
			f.c_novo.focus();
			return false;
			}
		}
	...
	if (f.rb_op[1].checked) {
		s_dest="fabricanteedita.asp";
		s_op=OP_CONSULTA;
		s_fabricante=f.c_cons.value;
		if (trim(f.c_cons.value)=="") {
			alert("Especifique o número do fabricante a ser consultado!!");
			f.c_cons.focus();
			return false;
			}
		}
...
<span class="T">CADASTRO DE FABRICANTES</span>
<div class="QFn" align="center">
<table class="TFn">
	<tr>
		<td nowrap>
			<input type="radio" id="rb_op" name="rb_op" value="1" class="CBOX" onclick="fOP.c_novo.focus()">
				<span style="cursor:default" onclick="fOP.rb_op[0].click(); fOP.c_novo.focus();">Cadastrar Novo</span>&nbsp;
				<input name="c_novo" id="c_novo" type="text" maxlength="3" size="6" onblur="this.value=normaliza_codigo(this.value,TAM_MIN_FABRICANTE);"
			...
			<input type="radio" id="rb_op" name="rb_op" value="2" class="CBOX" onclick="fOP.c_cons.focus()">
				...
				<input name="c_cons" id="c_cons" type="text" maxlength="3" size="6" onblur="this.value=normaliza_codigo(this.value,TAM_MIN_FABRICANTE);"
