		...
		if alerta = "" then
			with v_item(intCounter)
				if (Trim(.pedido) <> "") Or (Trim(.num_NF) <> "") then
					if (.vl_frete < 0) then
						...
						if Trim(.pedido) <> "" then
							...
							.msg_erro = texto_add_br(.msg_erro)
							.msg_erro = .msg_erro & "Valor de frete inválido para a NF " & Trim(.num_NF) & " (" & formata_moeda(.vl_frete) & ")"
							end if
						end if
					...
					if Trim(.num_NF) <> "" then
						...
						if n_pedido = 0 then
							...
							if Not blnNfCadastrada then
								.msg_erro = texto_add_br(.msg_erro)
								.msg_erro = .msg_erro & "A NF nº " & Trim(.num_NF) & " não está cadastrada!"
							else
								if Trim(.pedido) = "" then
									.msg_erro = texto_add_br(.msg_erro)
									.msg_erro = .msg_erro & "A NF nº " & Trim(.num_NF) & " não está vinculada a nenhum pedido!"
								else
									.msg_erro = texto_add_br(.msg_erro)
									.msg_erro = .msg_erro & "A NF nº " & Trim(.num_NF) & " não está vinculada ao pedido " & Trim(.pedido) & "!"
									end if
								end if
							end if
						...
						if n_pedido = 1 then
							if Trim(.pedido) = "" then
								...
								if Trim(.pedido) <> strPedido then
									blnErroFatal = True
									.msg_erro = texto_add_br(.msg_erro)
									.msg_erro = .msg_erro & "A NF " & Trim(.num_NF) & " refere-se a outro pedido (" & strPedido & ")"
									end if
								end if
							end if
						...
						if n_pedido > 1 then
							blnErroFatal = True
							.msg_erro = texto_add_br(.msg_erro)
							.msg_erro = .msg_erro & "A NF " & Trim(.num_NF) & " está associada a mais do que um pedido (" & strListaPedidos & ")"
							end if
				...
				'	SE NÃO FOI POSSÍVEL DETERMINAR O Nº PEDIDO ATRAVÉS DA NF, ASSEGURA QUE HAVERÁ UM MENSAGEM DE ERRO
					if Trim(.pedido) = "" then
						if Trim(.msg_erro) = "" then .msg_erro = "Falha ao tentar determinar o nº do pedido!"
						end if
                            ...
                            if rs.Eof then
                                ...
                                do while Not rs.Eof
                                    if (.vl_frete = rs("vl_frete")) And (.tipo_frete = Trim("" & rs("codigo_tipo_frete"))) And ((Ucase(c_transportadora) = Trim("" & rs("transportadora_id")) And Trim("" & rs("transportadora_id"))<>"")) then
                                        blnErroFatal = True
                                        .msg_erro = texto_add_br(.msg_erro)
								        .msg_erro = .msg_erro & "Pedido " & Trim(.pedido) & " já possui o valor de frete de " & SIMBOLO_MONETARIO & " " & formata_moeda(rs("vl_frete")) & " como " & obtem_descricao_tabela_t_codigo_descricao(GRUPO_T_CODIGO_DESCRICAO__PEDIDO_TIPO_FRETE, rs("codigo_tipo_frete")) & " e transportadora " & Trim("" & rs("transportadora_id"))
                                    end if
                                    if (.tipo_frete = Trim("" & rs("codigo_tipo_frete"))) And ((Ucase(c_transportadora) = Trim("" & rs("transportadora_id")) And Trim("" & rs("transportadora_id"))<>"")) And (.msg_alerta="") then
								        ...
								        .msg_alerta = .msg_alerta & "Pedido " & Trim(.pedido) & " já possui frete cadastrado como " & obtem_descricao_tabela_t_codigo_descricao(GRUPO_T_CODIGO_DESCRICAO__PEDIDO_TIPO_FRETE, rs("codigo_tipo_frete")) & " e transportadora " & Trim("" & rs("transportadora_id"))
                                    end if
				...
				if .num_NF <> "" then
					for intCounterAux=Lbound(v_item) to (intCounter-1)
						if (.num_NF = v_item(intCounterAux).num_NF) then
							alerta=texto_add_br(alerta)
							alerta=alerta & "NF Nº " & .num_NF & ": linha " & renumera_com_base1(Lbound(v_item),intCounter) & " repete a mesma NF da linha " & renumera_com_base1(Lbound(v_item),intCounterAux) & "."
							exit for
							end if
						next
					end if
...
<input type="hidden" name="c_url_origem" id="c_url_origem" value="PedidoAnotaFrete.asp" />
...
<table class="Qx" cellSpacing="0">
	...
	<tr>
		<td>&nbsp;</td>
		<td colspan="8" class="ME MD MB" align="left"><span class="PLTe">CD: </span><span class="C"><%=obtem_apelido_empresa_NFe_emitente(rNfeEmitente.id)%></span></td>
	</tr>
	<tr>
		<td colspan="9">&nbsp;</td>
	</tr>
...
<!--  VALOR TOTAL NF  -->
	<td class="MDB CelVlTotalNf" align="right">
		<span class="Cd"><%=formata_moeda(.vl_total_NF)%></span>
		</td>
