...
<script language="JavaScript" type="text/javascript">
	$(document).ready(function() {
		if ((true) || ($("input[name='rb_saida']:checked").val() == COD_SAIDA_REL_UF) || ($("input[name='rb_saida']:checked").val() == COD_SAIDA_REL_VENDEDOR)) {
			$("#tr_FormaPagto").show();
		}
		else {
			$("#tr_FormaPagto").hide();
		...
		$("input[name='rb_periodo']").change(function() {
		    ...
		    if ($("input[name='rb_periodo']:checked").val() == COD_CONSULTA_POR_PERIODO_ENTREGA) {
		    	$("#RowFiltroCst").show();
		    }
		    else {
		    	$("#RowFiltroCst").hide();
		...
		$("input[name='rb_tipo_cliente']").change(function() {
			if ($("input[name='rb_tipo_cliente']:checked").val() == "<%=ID_PF%>") {
				$("#DivContribIcms").hide();
			}
			else {
				$("#DivContribIcms").show();
...
<!--  PRODUTO  -->
	...
	<br>
		<input maxlength="13" class="PLLe" style="width:100px;" name="c_produto" id="c_produto" onblur="this.value=ucase(normaliza_codigo(this.value,TAM_MIN_PRODUTO));" onkeypress="if (digitou_enter(true)) $(this).hUtil('focusNext'); filtra_produto();">
		</td></tr>

<!--  CST  -->
	<tr bgcolor="#FFFFFF" id="RowFiltroCst">
	<td class="MDBE" align="left" nowrap><span class="PLTe">CST</span>
		<br>
		<input class="PLLe" style="width:200px;" name="c_cst" id="c_cst" onkeypress="if (digitou_enter(true)) fFILTRO.c_vendedor.focus();" onblur="this.value=trim(this.value);" title="Separar os códigos de CST com espaço ou vírgula" />
	</td>
	</tr>
...
<!--  FORMA DE PAGAMENTO  -->
	<tr bgcolor="#FFFFFF" id="tr_FormaPagto">
...
<!--  TIPO DE CLIENTE  -->
	...
	<td class="MDBE">
		...
		<tr>
			<td width="50%" align="left" valign="top" nowrap><span class="PLTe">TIPO DE CLIENTE</span>
			<br>
			<% intIdx=-1 %>
		...
		<td width="50%" class="ME" id="TdContribIcms">
			<div id="DivContribIcms">
			<span class="PLTe">CONTRIBUINTE ICMS</span>
			<br />
			<input type="checkbox" id="ckb_contribuinte_icms_sim" name="ckb_contribuinte_icms_sim" value="<%=COD_ST_CLIENTE_CONTRIBUINTE_ICMS_SIM%>" />
				<span class="C" style="cursor:default" onclick="fFILTRO.ckb_contribuinte_icms_sim.click();">Contribuinte</span>
			<br />
			<input type="checkbox" id="ckb_contribuinte_icms_nao" name="ckb_contribuinte_icms_nao" value="<%=COD_ST_CLIENTE_CONTRIBUINTE_ICMS_NAO%>" />
				<span class="C" style="cursor:default" onclick="fFILTRO.ckb_contribuinte_icms_nao.click();">Não Contribuinte</span>
			<br />
			<input type="checkbox" id="ckb_contribuinte_icms_isento" name="ckb_contribuinte_icms_isento" value="<%=COD_ST_CLIENTE_CONTRIBUINTE_ICMS_ISENTO%>" />
				<span class="C" style="cursor:default" onclick="fFILTRO.ckb_contribuinte_icms_isento.click();">Isento</span>
			</div>
		</td>
		</tr>
		</table>
...
<!--  LOJA  -->
	...
	<td class="MDBE" align="left" nowrap>
		...
		<tr>
			<td valign="top" width="50%" class="MD" align="left">
			...
			<td valign="top" align="left">
				...
				<br>
                    ...
                    <br />
                    <input type="radio" id="rb_saida" name="rb_saida" value="<%=COD_SAIDA_REL_LOJA%>"  class="CBOX" style="margin-left:20px;">
					<% intIdx=intIdx+1 %>
					<span style="cursor:default" class="rbLink" onclick="fFILTRO.rb_saida[<%=Cstr(intIdx)%>].click();">Loja</span>
                    <br />
                    ...
                    <br />
                    <input type="radio" id="rb_saida" name="rb_saida" value="<%=COD_SAIDA_REL_GRUPO_PRODUTO%>" checked class="CBOX" style="margin-left:20px;">
					<% intIdx=intIdx+1 %>
					<span style="cursor:default" class="rbLink" onclick="fFILTRO.rb_saida[<%=Cstr(intIdx)%>].click();">Grupo de Produtos</span>
                    &nbsp;&nbsp;
