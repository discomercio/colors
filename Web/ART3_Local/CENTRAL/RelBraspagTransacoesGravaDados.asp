...
<%
'     =================================================================
'	  RelBraspagTransacoesGravaDados.asp
'     =================================================================
'
'
...
'	=========================
	...
	'	===========
		for i=Lbound(v_transacao_tratada) to Ubound(v_transacao_tratada)
			if (v_transacao_tratada(i).id_registro <> "") then
				s = "SELECT * FROM t_PEDIDO_PAGTO_BRASPAG WHERE (id = " & v_transacao_tratada(i).id_registro & ")"
				...
				if rs.Eof then
					...
					'	INFORMAÇÕES PARA O LOG
						...
						s_log = s_log & v_transacao_tratada(i).pedido & " (t_PEDIDO_PAGTO_BRASPAG.id=" & v_transacao_tratada(i).id_registro & "; BraspagTransactionId=" & v_transacao_tratada(i).BraspagTransactionId & ")"
						end if
					end if
				end if
		...
		if alerta = "" then
			if s_log <> "" then
				s_log = "Transações Braspag marcadas como já tratadas: " & s_log
				grava_log usuario, "", "", "", OP_LOG_TRANSACOES_BRASPAG_MARCAR_COMO_JA_TRATADO, s_log
				end if
			end if
	...
	'	GRAVA ALTERAÇÕES NO TEXTO DA OBSERVAÇÃO EM TRANSAÇÕES MARCADAS COMO 'JÁ TRATADAS' ANTERIORMENTE
		if alerta = "" then
			for i=Lbound(v_transacao_editada) to Ubound(v_transacao_editada)
				if (v_transacao_editada(i).id_registro <> "") then
					s = "SELECT * FROM t_PEDIDO_PAGTO_BRASPAG WHERE (id = " & v_transacao_editada(i).id_registro & ")"
					if rs.State <> 0 then rs.Close
					rs.Open s, cn
					if rs.Eof then
						alerta=texto_add_br(alerta)
						alerta=alerta & "Registro da transação não foi encontrado (ID=" & v_transacao_editada(i).id_registro & ")."
					else
						s_texto_obs_original = Trim("" & rs("tratado_manual_obs"))
						v_transacao_editada(i).pedido = Trim("" & rs("pedido"))
						...
						if alerta = "" then
						'	LOG
							s_log_edicao = "Alteração na observação da transação t_PEDIDO_PAGTO_BRASPAG.id=" & v_transacao_editada(i).id_registro & ": " & s_texto_obs_original & " => " & v_transacao_editada(i).obs
							grava_log usuario, "", "", "", OP_LOG_TRANSACOES_BRASPAG_OBS_EDITADO, s_log_edicao
							end if
						end if
					end if
...
function fRetornar(f) {
	f.action = "RelBraspagTransacoesExec.asp?url_back=X";
	f.submit();
}
</script>
...
<tr>
	<td align="right" valign="bottom"><span class="PEDIDO">Transações Braspag<span class="C">&nbsp;</span></span></td>
</tr>
</table>
<br>
<br>
