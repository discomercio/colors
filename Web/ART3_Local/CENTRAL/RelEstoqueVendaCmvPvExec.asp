...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
    ...
    dim c_fabricante_multiplo, c_grupo, c_potencia_BTU, c_ciclo, c_posicao_mercado, v_fabricantes, v_grupos
    ...
    if alerta = "" then
		call set_default_valor_texto_bd(usuario, "RelEstoqueVendaCmvPvCentral|rb_detalhe", rb_detalhe)
		call set_default_valor_texto_bd(usuario, "RelEstoqueVendaCmvPvCentral|rb_exportacao", rb_exportacao)
		end if	
    ...
    if rb_exportacao = "Compostos" then
		...
		do while Not r.Eof
			...
			do while Not tPCI.Eof
                ...
                if c_potencia_BTU <> "" then
		            s_sql = s_sql & _
			            " AND (tPL.potencia_BTU = " & c_potencia_BTU & ")"
		            end if

                if c_ciclo <> "" then
		            s_sql = s_sql & _
			            " AND (tPL.ciclo = '" & c_ciclo & "')"
		            end if
	
	            if c_posicao_mercado <> "" then
		            s_sql = s_sql & _
			            " AND (tPL.posicao_mercado = '" & c_posicao_mercado & "')"
		            end if
