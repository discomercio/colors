	...
	for i=Lbound(v_lista) to Ubound(v_lista)
		if Trim(v_lista(i)) <> "" then
			...
			if (LBound(v) = UBound(v)) Or (Trim("" & v(LBound(v))) = "") Or (Trim("" & v(UBound(v))) = "") then
				...
				alerta=alerta & "Relação de pedidos com formato inválido na linha " & Cstr(i+1) & ": " & v_lista(i)
			else
	...
	for i=LBound(v_pedido) to UBound(v_pedido)
		if (Trim("" & v_pedido(i).pedido_marketplace) = "") And (Trim("" & v_pedido(i).pedido_ERP) = "") then
			...
			if (Trim("" & v_pedido(i).pedido_marketplace) <> "") And (Trim("" & v_pedido(i).pedido_ERP) <> "") then
				...
				alerta=texto_add_br(alerta)
				alerta=alerta & "Falha ao tentar estabelecer o vínculo entre o nº pedido marketplace e ERP (nº marketplace = " & Trim("" & v_pedido(i).pedido_marketplace) & ", nº ERP = " & Trim("" & v_pedido(i).pedido_ERP) & ")"
				end if
			end if
		next
				...
				if vl_TotalFamiliaPrecoNF >= (vl_TotalFamiliaPago + vl_TotalFamiliaDevolucaoPrecoNF) then
					...
					if Err <> 0 then
						...
						'	INFORMAÇÕES PARA O LOG
							...
							s_log = s_log & v_pedido(i).pedido_ERP & " (nº marketplace: " & v_pedido(i).pedido_marketplace & ")" & " = " & SIMBOLO_MONETARIO & " " & formata_moeda(vl_TotalFamiliaPrecoNF-vl_TotalFamiliaPago-vl_TotalFamiliaDevolucaoPrecoNF)
						'	ANÁLISE DE CRÉDITO
