...
<%
'     =================================================================
'	  RelComissaoIndicadoresPagDescGravaDados.asp
'     =================================================================
'
...
'	LAÇO DE TENTATIVAS PARA GERAR O NSU (DEVIDO A ACESSO CONCORRENTE)
    ...
    do while not rs.Eof
        ...
        if rs2.Eof then
            vl_total_comissao = rs("vl_total_comissao_liquido_arredondado_NFS")
            vl_total_RA = rs("vl_total_RA_liquido_arredondado_NFS")

            ' verifica se o RA é negativo, se sim, subtrair esse valor da comissão
            if rs("vl_total_RA_liquido_NFS")<0 then vl_total_comissao = vl_total_comissao + rs("vl_total_RA_liquido_NFS")            
            ...
            if rs("vl_total_comissao_liquido_NFS")<0 then vl_total_RA = vl_total_RA + rs("vl_total_comissao_liquido_NFS")
            if rs("meio_pagto") = "DIN" then
                vl_total_RA = o.digitoFinal(vl_total_RA)
            else
                vl_total_RA = floor(vl_total_RA)
            end if
            if rs("vl_total_comissao_liquido_arredondado_NFS")>0 then
            ...
            if rs("vl_total_RA_liquido_arredondado_NFS")>0 then
                 if Not fin_gera_nsu_fluxo_caixa_dis(T_FIN_FLUXO_CAIXA, intNsuNovoFluxoCaixa, msg_erro) then 
			            alerta = "FALHA AO GERAR NSU PARA O NOVO REGISTRO (" & msg_erro & ")"
		            else
			            if intNsuNovoFluxoCaixa <= 0 then
...
function fRetornar(f) {
	f.action = "RelComissaoIndicadoresPagDesc.asp";
