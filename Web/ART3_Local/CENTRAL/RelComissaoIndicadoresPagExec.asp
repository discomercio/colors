	...
	if aviso = "" then
		...
		do while Not r.Eof
			...
			if Trim("" & r("indicador")) <> indicador_a then
				...
				if (sub_total_com <= 0) Or (sub_total_com > 300) Or (banco = "237") Or (banco = "077") then
					if ( (sub_total_com > 0) Or (sub_total_com > 300) ) And ( (banco <> "237") And (banco <> "077") ) then
					...
					for cont=0 to Ubound(v_cedulas)
						...
						next
					end if 'if (sub_total_com <= 0) Or (sub_total_com > 300) Or (banco = "237") Or (banco = "077") then-else
				...
				if n_reg_total > 0 then
					...
					if Not rs2.Eof then
						...
						msg_desconto = ""
						end if 'if Not rs2.Eof then-else
					...
					if (vl_sub_total_RA_arredondado > 0) and (vl_sub_total_RT_arredondado < 0) then
						...
						if (vl_sub_total_RA_arredondado >= 0) And (vl_sub_total_RT_arredondado >= 0) then
							x = x & "       <input type='hidden' id='sub_total_comissaoRT_" & idx_bloco  - 1& "' value='" & Clng(vl_sub_total_RT_arredondado) & "'>" & chr(13)
						else
							x = x & "       <input type='hidden' id='sub_total_comissaoRT_" & idx_bloco - 1 & "' value='" & Clng(sub_total_com) & "'>" & chr(13)
							end if
						end if
					...
					if (vl_sub_total_RA_arredondado < 0) And (vl_sub_total_RT_arredondado > 0) then
						...
						if (vl_sub_total_RA_arredondado >= 0) And (vl_sub_total_RT_arredondado >= 0) then
							x = x & "       <input type='hidden' id='sub_total_comissaoRA_" & idx_bloco  - 1& "' value='" & Clng(vl_sub_total_RA_arredondado) & "'>" & chr(13)
						else
							x = x & "       <input type='hidden' id='sub_total_comissaoRA_" & idx_bloco  - 1& "' value='" & Clng(sub_total_com) & "'>" & chr(13)
							end if
						end if
					...
					elseif (sub_total_com < 301) And (sub_total_com >= 0) then
						x = x & "       <td align='left' colspan='5' nowrap>" & chr(13)
						...
						x = x &  " <td align='right' colspan='2' nowrap><span class='Rd' style='color:" & s_cor & ";'>"& formata_moeda(vl_sub_total_RA_arredondado)& "</span>" & chr(13)
						end if 'elseif (sub_total_com < 301) And (sub_total_com >= 0) then
					...
					elseif (sub_total_com > 300) And ( (banco <> "237") And (banco <> "077") ) then
						...
						if (vl_sub_total_RT_arredondado > 0) And (vl_sub_total_RA_arredondado < 0) then
							x = x & "       <input type='hidden'  id='forma_pag_valor_" & idx_bloco - 1 & "' value='"& sub_total_com & "'>" & chr(13)
							if vl_total_pagto <> "" then vl_total_pagto = vl_total_pagto & ";"
							vl_total_pagto = vl_total_pagto & sub_total_com
				...
				x = x & _
					"		<td class='MDTE' colspan='11' align='left' valign='bottom' class='MB' style='background:azure;'><span class='N'>&nbsp;" & s & "</span></td>" & chr(13) & _
					"		<td class='notPrint BkgWhite'>&nbsp;</td>" & chr(13) & _
					"	</tr>" & chr(13)
				
				if Trim("" & rs("opcao_dados_bancarios")) = Cstr(COD_ORCAMENTISTA_INDICADOR__OP_DADOS_BANCARIOS__CHAVE_PIX) then
					sChavePix = ""
					sDescricaoPix = consulta_descricao_vetor_t_codigo_descricao(vCodDescrTipoChavePix, GRUPO_T_CODIGO_DESCRICAO__ORCAMENTISTA_INDICADOR__PIX_TIPO_CHAVE, rs("pix_tipo_chave"), blnDescricaoPixCadastrada)
					...
					x = x & _
						...
						"				<tr>" & chr(13) & _
						"					<td align='left' class='MD' valign='bottom' style='height:15px;vertical-align:middle' width='33%'><span class='Cn'>Tipo Chave Pix: " & sDescricaoPix & "</span></td>" & chr(13) & _
						"					<td align='left' valign='bottom' style='height:15px;vertical-align:middle' width='67%'><span class='Cn'>Chave Pix: " & sChavePix & "</span></td>" & chr(13) & _
						"				</tr>" & chr(13) & _
						"				<tr>" & chr(13) & _
						"					<td colspan='2' class='MC' align='left' valign='bottom' style='height:15px;vertical-align:middle'><span class='Cn'>Favorecido Pix: " & Trim("" & rs("pix_favorecido")) & "</span></td>" & chr(13) & _
						"				</tr>" & chr(13) & _
						"			</table>" & chr(13) & _
						"		</td>" & chr(13) & _
						"		<td class='notPrint BkgWhite'>&nbsp;</td>" & chr(13) & _
				...
				vl_sub_total_RA_liquido = 0
				vl_sub_total_RA_diferenca = 0
				end if 'if Trim("" & r("indicador")) <> indicador_a then
			...
			if (((vl_sub_total_RT >= 0) And (vl_sub_total_RA_liquido >= 0)) And (((vl_sub_total_RT + vl_sub_total_RA_liquido) < 301) And ((banco <> "237") Or (banco <> "077")))) then
		...
		if n_reg <> 0 then
			...
			if (vl_sub_total_RA_arredondado >= 0) And (vl_sub_total_RT_arredondado >= 0) then
				x = x & "       <input type='hidden' id='sub_total_comissaoRT_" & idx_bloco & "' value='" & Clng(vl_sub_total_RT_arredondado) & "'>" & chr(13)
			else
				if (vl_sub_total_RA_arredondado >= 0) And (vl_sub_total_RT_arredondado < 0) then
					x = x & "       <input type='hidden' id='sub_total_comissaoRT_" & idx_bloco & "' value='" & Clng(0) & "'>" & chr(13)
				else
					x = x & "       <input type='hidden' id='sub_total_comissaoRT_" & idx_bloco & "' value='" & Clng(sub_total_com) & "'>" & chr(13)
					end if
				end if
			...
			if (vl_sub_total_RA_arredondado >= 0) And (vl_sub_total_RT_arredondado >= 0) then
				x = x & "       <input type='hidden' id='sub_total_comissaoRA_" & idx_bloco & "' value='" & Clng(vl_sub_total_RA_arredondado) & "'>" & chr(13)
			else
				if (vl_sub_total_RA_arredondado < 0) And (vl_sub_total_RT_arredondado > 0) then
					x = x & "       <input type='hidden' id='sub_total_comissaoRA_" & idx_bloco & "' value='" & Clng(0) & "'>" & chr(13)
				else
					x = x & "       <input type='hidden' id='sub_total_comissaoRA_" & idx_bloco & "' value='" & Clng(sub_total_com) & "'>" & chr(13)
					end if
				end if
			...
			elseif (sub_total_com > 300) And (banco <> "237") And (banco <> "077") then
				...
				if (vl_sub_total_RT_arredondado > 0) And (vl_sub_total_RA_arredondado < 0) then
					x = x & "       <input type='hidden' id='forma_pag_valor_" & idx_bloco & "' value='" & sub_total_com & "'>" & chr(13)
					if vl_total_pagto <> "" then vl_total_pagto = vl_total_pagto & ";"
					vl_total_pagto = vl_total_pagto & sub_total_com
				elseif (vl_sub_total_RA_arredondado > 0) And (vl_sub_total_RT_arredondado < 0) then
					x = x & "       <input type='hidden' id='forma_pag_valor_" & idx_bloco & "' value='" & sub_total_com & "'>" & chr(13)
			...
			elseif (banco = "237") Or (banco = "077") then
				...
				end if 'if sub_total_com <= 0 then-else
			...
			if qtde_indicadores >= 1 then
				...
				if (sub_total_com >= 0) And (valor_desconto = 0) then
					s_checked=" checked"
				else
					s_disabled = " disabled"
					cod_motivo_negativo = cod_motivo_negativo & ind_anterior & ", "
