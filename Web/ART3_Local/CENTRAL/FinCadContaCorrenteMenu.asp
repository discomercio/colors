...
var s_dest, s_op, s_id;
	...
	if (f.rb_op[0].checked) {
		s_dest="FinCadContaCorrenteEdita.asp";
		s_op=OP_INCLUI;
		s_id=f.c_novo.value;
		if (trim(f.c_novo.value)=="") {
			alert("Informe o código de identificação para a nova conta corrente!!");
			f.c_novo.focus();
			return false;
			}
		}
	...
	if (f.rb_op[1].checked) {
		...
		if (trim(f.c_cons.value)=="") {
			alert("Informe o código de identificação da conta corrente a ser consultada!!");
			f.c_cons.focus();
			return false;
			}
		}
	...
	if (f.rb_op[2].checked) {
		s_dest="FinCadContaCorrenteLista.asp";
		}
...
<span class="T">CADASTRO DE CONTA CORRENTE</span>
	...
	<tr>
		<td nowrap>
			<input type="radio" id="rb_op" name="rb_op" value="1" class="CBOX" onclick="fOP.c_novo.focus()">
				...
				<input name="c_novo" id="c_novo" type="text" maxlength="2" size="6" 
			...
			<input type="radio" id="rb_op" name="rb_op" value="2" class="CBOX" onclick="fOP.c_cons.focus();">
				...
				<input name="c_cons" id="c_cons" type="text" maxlength="1" size="6" 
					onblur="this.value=trim(this.value);" 
