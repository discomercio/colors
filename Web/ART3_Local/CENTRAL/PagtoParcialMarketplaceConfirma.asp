		...
		if v_dados_pagto(i) <> "" then
			...
			with v_pedido(Ubound(v_pedido))
				...
				if .pedido_marketplace = "" then
					...
					alerta=alerta & "Nº pedido marketplace não informado."
					end if

				if .pedido_ERP = "" then
					alerta=texto_add_br(alerta)
					alerta=alerta & "Nº pedido ERP não informado."
					end if
				
				if .vl_parcela = 0 then
					alerta=texto_add_br(alerta)
					alerta=alerta & "Valor do pagamento parcial é inválido (pedido marketplace nº " & pedido_marketplace & ")"
					end if
				end with
			end if
		next
			...
			if (alerta = "") And (v_pedido(iPedido).pedido_ERP <> "") then
				...
				with v_pedido(iPedido)
					...
					if Err <> 0 then
						...
						rs.Open s, cn
						if rs.Eof then
							alerta = "Pedido-base " & .pedido_base_ERP & " não foi encontrado."
						else
							...
							'	ANÁLISE DE CRÉDITO
								if Trim("" & rs("loja")) = NUMERO_LOJA_ECOMMERCE_AR_CLUBE then
									if (CLng(rs("analise_credito")) = CLng(COD_AN_CREDITO_ST_INICIAL)) then
										...
										s_log_aux = s_log_aux & " Análise de crédito: " & descricao_analise_credito(rs("analise_credito")) & " => " & descricao_analise_credito(COD_AN_CREDITO_OK)
								...
								'	AUTORIZANDO A TRANSAÇÃO. NESTE CASO, O REGISTRO DO PAGAMENTO PRECISA SER FEITO MANUALMENTE NO PEDIDO.
									...
									'	SE HOUVER PARCELA PAGA EM CARTÃO, A EQUIPE DA ANÁLISE DE CRÉDITO DEVE VERIFICAR A NECESSIDADE DE PEDIR DOCUMENTAÇÃO COMPLEMENTAR AO CLIENTE
										If (intNumParcelasPagtoCartao = 0) then
											...
											if s_log_aux <> "" then s_log_aux = s_log_aux & "; "
											s_log_aux = s_log_aux & " Análise de crédito: " & descricao_analise_credito(rs("analise_credito")) & " => " & descricao_analise_credito(COD_AN_CREDITO_PENDENTE_VENDAS)
