...
<%
'     =================================================================
'	  RelPedidosNaoRecebidosGravaDados.asp
'     =================================================================
'
'
...
'	OBTÉM DADOS DO FORMULÁRIO
	dim i, j, n, c, vAux, s_pedido, s_dt_recebimento, dt_recebimento, s_dados_aux
...
'	=========================
	...
	'	===========
				...
				if rs.Eof then
					...
					if Trim("" & rs("st_entrega"))<>ST_ENTREGA_ENTREGUE then
						...
						if CLng(rs("PedidoRecebidoStatus")) = CLng(COD_ST_PEDIDO_RECEBIDO_SIM) then
							alerta=texto_add_br(alerta)
							alerta=alerta & "Pedido " & v_pedidos(i) & " já está assinalado como pedido recebido pelo cliente (" & formata_data(rs("PedidoRecebidoData")) & ")"
						else
							rs("PedidoRecebidoStatus") = CLng(COD_ST_PEDIDO_RECEBIDO_SIM)
							rs("PedidoRecebidoData") = dt_recebimento
		...
		if alerta = "" then
			s_log = "Operação=Recebido (" & s_dt_recebimento & ")"
...
function fRetornar(f) {
	f.action = "RelPedidosNaoRecebidosFiltro.asp?url_back=X";
	f.submit();
}
</script>
...
<tr>
	<td align="right" valign="bottom"><span class="PEDIDO">Pedidos Não Recebidos Pelo Cliente<span class="C">&nbsp;</span></span>
		<br><span class="Rc">
