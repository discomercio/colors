...
<tr>
	<td align="right" valign="bottom"><p class="PEDIDO">Estoque XML (alteração)<span class="C">&nbsp;</span></p></td>
</tr>
</table>
<br>
...
<table id="tableProduto" class="Qx" cellspacing="0">
	...
	<tr bgcolor="#FFFFFF">
	<th>&nbsp;</th>
	<td class="MB TdErpCodigo" align="left" style="vertical-align:bottom;"><span class="PLTe">CÓD</span></td>
...
%>
	...
	<td class="MDB" align="right">
		<input name="c_vl_unitario" id="c_vl_unitario" <%=s_valor_readonly%> class="PLLd" style="width:62px;"
			...
			onblur="this.value=formata_moeda(this.value); if (converte_numero(this.value)<0) {alert('Valor inválido!!');this.focus();} else {recalcula_itens();}"
			/></td>
	<td class="MDB" align="right">
		<input name="c_vl_custo2" id="c_vl_custo2" <%=s_valor_readonly%> class="PLLd" style="width:62px;"
			value="<%=s_vl_custo2%>"
			onkeypress="if (digitou_enter(true)) $(this).hUtil('focusNext'); filtra_moeda();"
			onblur="this.value=formata_moeda(this.value); if (converte_numero(this.value)<0) {alert('Valor inválido!!');this.focus();} else {recalcula_total(<%=Cstr(i)%>);}"
			/>
		</td>
	<td class="MDB" align="right">
