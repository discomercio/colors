...
'	FORAM CANCELADOS E QUE ESTÃO NO BD APENAS POR QUESTÃO DE HISTÓRICO.
  ...
  ' CABEÇALHO
	...
	cab = "	<TR style='background:azure' NOWRAP>" & chr(13) & _
		  "		<TD width='75' valign='bottom' NOWRAP class='MDBE'><P class='R'>PRODUTO</P></TD>" & chr(13) & _
			...
			if n_reg_total > 0 then 
			  ' FECHA TABELA DA LOJA ANTERIOR
				x = x & "	<TR NOWRAP>" & chr(13) & _
						...
						"Total:" & "</P></TD>" & chr(13) & _
						"		<TD NOWRAP class='MDB'><P class='Cd'>" & formata_inteiro(n_saldo_parcial) & _
						"</P></TD>" & chr(13) & _
						"	</TR>" & chr(13) & _
						"</TABLE>" & chr(13)
				Response.Write x
	...
	'	MUDOU DE FABRICANTE?
		...
		'	SUB-TOTAL POR FORNECEDOR
			if n_reg > 0 then
				x = x & "	<TR NOWRAP>" & chr(13) & _
						...
						"		<TD class='MDB'><P class='Cd'>" & formata_inteiro(n_saldo_parcial) & "</P></TD>" & chr(13) & _
						"	</TR>" & chr(13) & _
						"	<TR NOWRAP>" & chr(13) & _
	 ...
	 '> PRODUTO
 	 ...
 	 '> DESCRIÇÃO
		x = x & "		<TD class='MDB' valign='bottom' width='" & CStr(LargColDescricao) & "'><P class='C' NOWRAP>&nbsp;" & produto_formata_descricao_em_html(Trim("" & r("descricao_html"))) & "</P></TD>" & chr(13)
  ...
  ' MOSTRA TOTAL DA ÚLTIMA LOJA
	if n_reg <> 0 then 
		x = x & "	<TR NOWRAP>"  & chr(13) & _
				...
				"		<TD class='MDB' NOWRAP><P class='Cd'>" & formata_inteiro(n_saldo_parcial) & "</P></TD>" & chr(13) & _
				"	</TR>" & chr(13)
	'>	TOTAL GERAL
		if (qtde_lojas > 1) Or (qtde_fabricantes > 1) then
			...
			x = x & _
				...
				"		<TD class='MTBD' valign='bottom' NOWRAP><p class='Cd'>" & formata_inteiro(n_saldo_total) & "</p></TD>" & chr(13) & _
				"	</TR>" & chr(13)
			end if
		end if
  ...
  ' MOSTRA AVISO DE QUE NÃO HÁ DADOS!!
	if n_reg_total = 0 then
		x = cab_table & cab & _
			"	<TR NOWRAP>" & chr(13) & _
			"		<TD colspan='" & CStr(intQtdeTotalColunasColSpan) & "' class='MDBE'><P class='ALERTA'>&nbsp;NENHUM PRODUTO DO ESTOQUE SATISFAZ AS CONDIÇÕES ESPECIFICADAS&nbsp;</P></TD>" & chr(13) & _
			"	</TR>" & chr(13)
		end if
...
'	FORAM CANCELADOS E QUE ESTÃO NO BD APENAS POR QUESTÃO DE HISTÓRICO.
	s_sql = "SELECT loja, CONVERT(smallint,loja) AS numero_loja" & _
			...
			", Sum(t_ESTOQUE_MOVIMENTO.qtde*t_ESTOQUE_ITEM.preco_fabricante) AS preco_total" & _
...
'	FORAM CANCELADOS E QUE ESTÃO NO BD APENAS POR QUESTÃO DE HISTÓRICO.
	s_sql = "SELECT loja, CONVERT(smallint,loja) AS numero_loja" & _
			...
			", descricao_html" & _
			", Sum(t_ESTOQUE_MOVIMENTO.qtde) AS saldo" & _
			", t_ESTOQUE_ITEM.preco_fabricante" & _
			" FROM t_ESTOQUE_MOVIMENTO INNER JOIN t_ESTOQUE_ITEM ON ((t_ESTOQUE_MOVIMENTO.id_estoque=t_ESTOQUE_ITEM.id_estoque) AND (t_ESTOQUE_MOVIMENTO.fabricante=t_ESTOQUE_ITEM.fabricante) AND (t_ESTOQUE_MOVIMENTO.produto=t_ESTOQUE_ITEM.produto))" & _
			" LEFT JOIN t_PRODUTO ON ((t_ESTOQUE_MOVIMENTO.fabricante=t_PRODUTO.fabricante) AND (t_ESTOQUE_MOVIMENTO.produto=t_PRODUTO.produto))" & _
			" WHERE (anulado_status=0)" & _
			" AND (estoque='" & rb_estoque & "')"	
	...
	s_sql = s_sql & " GROUP BY loja, t_ESTOQUE_MOVIMENTO.fabricante, t_ESTOQUE_MOVIMENTO.produto, descricao, descricao_html, t_ESTOQUE_ITEM.preco_fabricante" & _
					" ORDER BY numero_loja, t_ESTOQUE_MOVIMENTO.fabricante, t_ESTOQUE_MOVIMENTO.produto, descricao, descricao_html, t_ESTOQUE_ITEM.preco_fabricante"
...
dim vl, vl_total_geral, vl_sub_total
	...
	s_sql = "SELECT t_ESTOQUE_ITEM.fabricante, t_ESTOQUE_ITEM.produto, descricao, descricao_html" & _
			", Sum(qtde-qtde_utilizada) AS saldo, Sum((qtde-qtde_utilizada)*t_ESTOQUE_ITEM.preco_fabricante) AS preco_total" & _
			" FROM t_ESTOQUE_ITEM LEFT JOIN t_PRODUTO ON" & _
			" ((t_ESTOQUE_ITEM.fabricante=t_PRODUTO.fabricante) AND (t_ESTOQUE_ITEM.produto=t_PRODUTO.produto))" & _
			" WHERE ((qtde-qtde_utilizada) > 0)"
...
dim vl, vl_total_geral, vl_sub_total
	...
	s_sql = "SELECT t_ESTOQUE_ITEM.fabricante, t_ESTOQUE_ITEM.produto, descricao, descricao_html" & _
			", Sum(qtde-qtde_utilizada) AS saldo, t_ESTOQUE_ITEM.preco_fabricante" & _
			" FROM t_ESTOQUE_ITEM LEFT JOIN t_PRODUTO ON" & _
			" ((t_ESTOQUE_ITEM.fabricante=t_PRODUTO.fabricante) AND (t_ESTOQUE_ITEM.produto=t_PRODUTO.produto))" & _
			" WHERE ((qtde-qtde_utilizada) > 0)"
	...
	s_sql = s_sql & " GROUP BY t_ESTOQUE_ITEM.fabricante, t_ESTOQUE_ITEM.produto, descricao, descricao_html, t_ESTOQUE_ITEM.preco_fabricante" & _
					" ORDER BY t_ESTOQUE_ITEM.fabricante, t_ESTOQUE_ITEM.produto, descricao, descricao_html, t_ESTOQUE_ITEM.preco_fabricante"
