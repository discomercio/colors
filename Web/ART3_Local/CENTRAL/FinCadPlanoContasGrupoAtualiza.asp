...
<%
'     ===========================================
'	  FinCadPlanoContasGrupoAtualiza.asp
'     ===========================================
'
'
...
'	OBTÉM DADOS DO FORMULÁRIO ANTERIOR
	...
	s_id=normaliza_codigo(s_id, TAM_PLANO_CONTAS__GRUPO)
	...
	elseif s_descricao = "" then
		alerta="PREENCHA A DESCRIÇÃO DO GRUPO DO PLANO DE CONTAS."
	elseif s_st_ativo = "" then
		alerta="INFORME O STATUS DO GRUPO DO PLANO DE CONTAS."
		end if
	...
	select case operacao_selecionada
		...
		'	 =========
			s = "SELECT" & _
				...
				" WHERE" & _
					" (id_plano_contas_grupo = " & s_id & ")"
			r.Open s, cn
			if Not r.Eof then
				erro_fatal=True
				alerta = "REGISTRO NÃO PODE SER REMOVIDO PORQUE ESTÁ SENDO REFERENCIADO NA TABELA DE PLANO DE CONTAS."
				end if
			r.Close 
			...
			'	APAGA!!
				...
				cn.Execute(s)
				If Err = 0 then 
					if s_log <> "" then grava_log usuario, "", "", "", OP_LOG_PLANO_CONTAS_GRUPO_EXCLUSAO, s_log
				else
					erro_fatal=True
			...
			if alerta = "" then 
				...
				If Err = 0 then 
					...
					if criou_novo_reg then
						...
						if s_log <> "" then grava_log usuario, "", "", "", OP_LOG_PLANO_CONTAS_GRUPO_INCLUSAO, s_log
					else
						s_log = log_via_vetor_monta_alteracao(vLog1, vLog2)
						if s_log <> "" then 
							s_log="Id=" & Trim("" & r("id")) & "; " & s_log
							grava_log usuario, "", "", "", OP_LOG_PLANO_CONTAS_GRUPO_ALTERACAO, s_log
							end if
						end if
				else
					erro_fatal=True
...
<tr>
<%
	s="FinCadPlanoContasGrupoMenu.asp" & "?" & MontaCampoQueryStringSessionCtrlInfo(Session("SessionCtrlInfo"))
