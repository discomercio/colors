...
'	VERIFICA ID
	dim s, usuario, usuario_nome, senha
...
'	OBTEM O ID
	...
	if (usuario="") or (senha="") then Response.Redirect("aviso.asp?id=" & ERR_SESSAO)
...
'	=========================
	...
	set rs = cn.execute("select dt_ult_alteracao_senha from t_USUARIO where usuario = '" & usuario & "'")
...
<div class="QFn" style="width:300px" align="center">
	...
	<input name="novasenha2" id="novasenha2" type="password" maxlength="15" style="text-align:center;" onkeypress="if (digitou_enter(true) && tem_info(this.value)) if (confere(fPWD)) fPWD.submit();">
	
	<p class="R" style="margin: 4 10 0 10">&nbsp;</p>
	<input name="CONSULTAR" id="CONSULTAR" type="button" class="Botao" 
		   value="CONFIRMAR" title="confirma a alteração da senha do usuário" onclick="if (confere(fPWD)) fPWD.submit();">
...
<% if primeira_vez then %>
	<td align="center"><a href="sessaoencerra.asp<%= "?" & MontaCampoQueryStringSessionCtrlInfo(Session("SessionCtrlInfo"))%>" title="cancela a alteração de senha e encerra a sessão">
