...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_CEN_REL_MULTICRITERIO_ORCAMENTOS, s_lista_operacoes_permitidas) then 
		cn.Close
	...
	dim rb_loja, c_loja, c_loja_de, c_loja_ate
	dim c_orcamento
...
'	CRITÉRIO: FORMA DE PAGAMENTO (NOVA VERSÃO)
	...
	if op_forma_pagto <> "" then
		s = " (t_ORCAMENTO.av_forma_pagto = " & op_forma_pagto & ")" & _
			" OR (t_ORCAMENTO.pu_forma_pagto = " & op_forma_pagto & ")" & _
			" OR (t_ORCAMENTO.pce_forma_pagto_entrada = " & op_forma_pagto & ")" & _
			" OR (t_ORCAMENTO.pce_forma_pagto_prestacao = " & op_forma_pagto & ")" & _
			" OR (t_ORCAMENTO.pse_forma_pagto_prim_prest = " & op_forma_pagto & ")" & _
			" OR (t_ORCAMENTO.pse_forma_pagto_demais_prest = " & op_forma_pagto & ")"
		if op_forma_pagto = ID_FORMA_PAGTO_CARTAO then
			s = s & " OR (t_ORCAMENTO.tipo_parcelamento = " & COD_FORMA_PAGTO_PARCELADO_CARTAO & ")"
		elseif op_forma_pagto = ID_FORMA_PAGTO_CARTAO_MAQUINETA then
			s = s & " OR (t_ORCAMENTO.tipo_parcelamento = " & COD_FORMA_PAGTO_PARCELADO_CARTAO_MAQUINETA & ")"
			end if
		end if
...
'	CRITÉRIO: QUANTIDADE DE PARCELAS
	...
	if c_forma_pagto_qtde_parc <> "" then
		s = " (t_ORCAMENTO.qtde_parcelas = " & c_forma_pagto_qtde_parc & ")"
		end if
...
'	CRITÉRIO: LOJA
	if rb_loja = "UMA" then
		if c_loja <> "" then
			s = " (CONVERT(smallint, t_ORCAMENTO.loja) = " & c_loja & ")"
			if s_where <> "" then s_where = s_where & " AND"
			s_where = s_where & " (" & s & ")"
			end if
	elseif rb_loja = "FAIXA" then
		...
		if c_loja_de <> "" then
			if s <> "" then s = s & " AND"
			s = s & " (CONVERT(smallint, t_ORCAMENTO.loja) >= " & c_loja_de & ")"
			end if

		if c_loja_ate <> "" then
			if s <> "" then s = s & " AND"
			s = s & " (CONVERT(smallint, t_ORCAMENTO.loja) <= " & c_loja_ate & ")"
			end if
...
'	CRITÉRIO: VENDEDOR
	if c_vendedor <> "" then
		if s_where <> "" then s_where = s_where & " AND"
		s_where = s_where & " (t_ORCAMENTO.vendedor = '" & c_vendedor & "')"
		end if
...
'	CRITÉRIO: INDICADOR (LEMBRE-SE: O ORÇAMENTISTA DE UM ORÇAMENTO É USADO AUTOMATICAMENTE COMO O INDICADOR DO PEDIDO QUANDO O ORÇAMENTO VIRA PEDIDO)
	if c_indicador <> "" then
		if s_where <> "" then s_where = s_where & " AND"
		s_where = s_where & " (t_ORCAMENTO.orcamentista = '" & c_indicador & "')"
		end if
...
'	MONTA CLÁUSULA FROM
  ...
  ' CABEÇALHO
	...
	cab = "	<TR style='background:azure'>" & chr(13) & _
		  ...
		  "		<TD class='MDTE' style='width:70px' valign='bottom' NOWRAP><P class='R'>Orçamento</P></TD>" & chr(13) & _
		  ...
		  "		<TD class='MTD' style='width:70px' valign='bottom'><P class='R'>Status do Orçamento</P></TD>" & chr(13) & _
		  "	</TR>" & chr(13)
	...
	'> Nº ORÇAMENTO
		x = x & "		<TD valign='top' class='MDTE'><P class='C'>&nbsp;<a href='javascript:fRELConcluir(" & _
				...
				")' title='clique para consultar o orçamento'>" & Trim("" & r("orcamento")) & "</a></P></TD>" & chr(13)
  ...
  ' MOSTRA AVISO DE QUE NÃO HÁ DADOS!!
	if n_reg_total = 0 then
		...
		x = x & "	<TR>" & chr(13) & _
				"		<TD valign='bottom' style='background:white;' NOWRAP>&nbsp;</TD>" & chr(13) & _
				"		<TD class='MT' colspan='5'><P class='ALERTA'>&nbsp;NENHUM ORÇAMENTO ENCONTRADO&nbsp;</P></TD>" & chr(13) & _
				"	</TR>" & chr(13)
		end if
...
<% 
	...
	if s <> "" then
		s_filtro = s_filtro & _
					"	<tr>" & chr(13) & _
					"		<td align='right' valign='top' NOWRAP><p class='N'>Status do Orçamento:&nbsp;</p></td>" & chr(13) & _
	...
	if (c_dt_cadastro_inicio <> "") Or (c_dt_cadastro_termino <> "") then
		...
		s_filtro = s_filtro & _
					"	<tr>" & chr(13) & _
					"		<td align='right' valign='top' NOWRAP><p class='N'>Orçamentos cadastrados entre:&nbsp;</p></td>" & chr(13) & _
					"		<td valign='top' width='99%'><p class='N'>" & s & "</p></td>" & chr(13) & _
					"	</tr>" & chr(13)
		end if
	...
	if ckb_produto <> "" then 
		...
		s_filtro = s_filtro & _
					"	<tr>" & chr(13) & _
					"		<td align='right' valign='top' NOWRAP><p class='N'>Somente orçamentos que incluam:&nbsp;</p></td>" & chr(13) & _
	...
	if c_orcamento <> "" then
		...
		s_filtro = s_filtro & _
					"	<tr>" & chr(13) & _
					"		<td align='right' valign='top' NOWRAP><p class='N'>Nº Orçamento:&nbsp;</p></td>" & chr(13) & _
					"		<td valign='top'><p class='N'>" & s & "</p></td>" & chr(13) & _
					"	</tr>" & chr(13)
		end if
