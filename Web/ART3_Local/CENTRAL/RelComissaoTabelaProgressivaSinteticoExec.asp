...
<%
'     ======================================================
'	  RelComissaoTabelaProgressivaSinteticoExec.asp
'     ======================================================
'
'
...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_CEN_REL_COMISSAO_VENDEDORES_TABELA_PROGRESSIVA_SINTETICO, s_lista_operacoes_permitidas) then 
		cn.Close
...
const COD_OPERACAO_PERDA = "PERDA"
dim r
dim s, s_aux, s_sql, x, cab_table, cab, vendedor_a, n_linhas, n_linhas_total, n_reg, n_reg_total, qtde_vendedores
dim perc_comissao, perc_desconto
dim vl_comissao_aux, vl_saida_aux, vl_lista_aux, vl_total_saida, vl_total_comissao, vl_sub_total_saida, vl_sub_total_comissao
dim vl_comissao_acumulado, vl_saida_acumulado
...
dim vTabela, intUltPosicaoTabela, intQtdeTabelas, rsTabela, rsPerfil, intCounter
dim strVendedorAtual, strLojaAtual, strPedidoAtual, strDataPedidoAtual, strOperacaoAtual
...
'	PERDAS
	...
	s_sql = s_sql & " UNION " & _
			"SELECT" & _
				" '" & COD_OPERACAO_PERDA & "' AS operacao," & _
				" t_PEDIDO.vendedor," & _
				" t_PEDIDO.loja AS loja," & _
				" CONVERT(smallint,t_PEDIDO.loja) AS numero_loja," & _
	...
	'	MUDOU DE VENDEDOR?
		...
		'	OBTÉM TABELA DE COMISSÃO
			...
			if s <> "" then x = x & "	<TR>" & chr(13) & _
									"		<TD class='MDTE' COLSPAN='6' valign='bottom' style='background:azure;'><p class='N'>&nbsp;" & s & "</p></td>" & chr(13) & _
									"	</tr>" & chr(13)
			x = x & cab
			
			if intQtdeTabelas = 0 then
				x = x & "	<TR>" & chr(13) & _
						"		<TD class='MDTE MB' COLSPAN='6' align='center' valign='bottom' style='background:red;'><p class='N' style='color:yellow'>&nbsp;Nenhuma tabela de comissão associada ao vendedor&nbsp;</p></td>" & chr(13) & _
						"	</tr>" & chr(13)
			elseif intQtdeTabelas > 1 then
				x = x & "	<TR>" & chr(13) & _
						"		<TD class='MDTE MB' COLSPAN='6' align='center' valign='bottom' style='background:red;'><p class='N' style='color:yellow'>&nbsp;Há " & Cstr(intQtdeTabelas) & " tabelas de comissão associadas ao vendedor&nbsp;</p></td>" & chr(13) & _
						"	</tr>" & chr(13)
				end if
			end if
	  ...
	  ' CONTAGEM
		if intQtdeTabelas <> 1 then
			...
			strVendedorAtual = Trim("" & r("vendedor"))
			strLojaAtual = Trim("" & r("loja"))
			strPedidoAtual = Trim("" & r("pedido"))
			strDataPedidoAtual = formata_data(r("data"))
			strOperacaoAtual = Trim("" & r("operacao"))

			vl_saida_acumulado = 0
			vl_comissao_acumulado = 0
		
		'	O CÁLCULO DA COMISSÃO PRECISA SER ITEM A ITEM, POIS O DESCONTO É DIFERENTE P/ CADA ITEM,
		'	PORTANTO, O PERCENTUAL DE COMISSÃO TAMBÉM FICA DIFERENTE.
		'	PARA CONSOLIDAR EM UMA VISÃO SINTÉTICA, PERCORRE OS ITENS SEGUINTES, CALCULA E ACUMULA
		'	OS VALORES.
		'	NESTA LÓGICA, AO SAIR DO LAÇO, O RECORDSET ESTARÁ POSICIONADO EM UM REGISTRO QUE PERTENCE:
		'		1) À PRÓXIMA LINHA DO RELATÓRIO
		'		2) AO PRÓXIMO VENDEDOR
		'		3) ATINGIU O EOF
			do while True
				n_reg = n_reg + 1
				n_reg_total = n_reg_total + 1
			...
			'	EVITA DIFERENÇAS DE ARREDONDAMENTO
				vl_saida_aux = converte_numero(formata_moeda(r("valor_saida")))
				vl_lista_aux = converte_numero(formata_moeda(r("valor_lista")))
	
			'	CALCULA COMISSÃO
				if vl_lista_aux = 0 then
					perc_desconto = 0
				else
					perc_desconto = 100*((vl_lista_aux - vl_saida_aux) / vl_lista_aux)
					end if
					
				if Trim("" & r("operacao")) = COD_OPERACAO_PERDA then
					perc_comissao = 0
					vl_comissao_aux = 0
				else
					perc_comissao = 0
					for intCounter = 0 to intUltPosicaoTabela
						...
						next
					vl_comissao_aux = converte_numero(formata_moeda((perc_comissao/100)*vl_saida_aux))
					end if
				
				vl_saida_acumulado = vl_saida_acumulado + vl_saida_aux
				vl_comissao_acumulado = vl_comissao_acumulado + vl_comissao_aux

			'	PRÓXIMO REGISTRO
				r.MoveNext
				
				if r.Eof then exit do
				if Trim("" & r("vendedor")) <> strVendedorAtual then exit do
				if Trim("" & r("loja")) <> strLojaAtual then exit do
				if Trim("" & r("pedido")) <> strPedidoAtual then exit do
				if formata_data(r("data")) <> strDataPedidoAtual then exit do
				if Trim("" & r("operacao")) <> strOperacaoAtual then exit do
				loop

			if (vl_saida_acumulado < 0) Or (vl_comissao_acumulado < 0) then
				s_cor = "red"
				s_cor_sinal = "red"
				s_sinal = "-"
			else
			...
			n_linhas = n_linhas + 1
			n_linhas_total = n_linhas_total + 1
			
			x = x & "	<TR NOWRAP>"  & chr(13)

		 '> LOJA
			x = x & "		<TD class='MDTE'><P class='Cnc' style='color:" & s_cor & ";'>" & strLojaAtual & "</P></TD>" & chr(13)

		 '> Nº PEDIDO
			x = x & "		<TD class='MTD'><P class='Cn'><a style='color:" & s_cor & ";' href='javascript:fRELConcluir(" & _
					chr(34) & strPedidoAtual & chr(34) & ")' title='clique para consultar o pedido'>" & _
					strPedidoAtual & "</a></P></TD>" & chr(13)

		 '> DATA DO PEDIDO
			x = x & "		<TD align='center' class='MTD'><P class='Cnc' style='color:" & s_cor & ";'>" & strDataPedidoAtual & "</P></TD>" & chr(13)

		 '> VALOR DO PEDIDO
			x = x & "		<TD align='right' class='MTD'><P class='Cnd' style='color:" & s_cor & ";'>" & formata_moeda(vl_saida_acumulado) & "</P></TD>" & chr(13)

		 '> VALOR DA COMISSÃO
			if strOperacaoAtual = COD_OPERACAO_PERDA then 
				s_vl_comissao = "&nbsp;"
			else
				s_vl_comissao = formata_moeda(vl_comissao_acumulado)
				end if
		 ...
		 '> +/-
			...
			vl_sub_total_saida = vl_sub_total_saida + vl_saida_acumulado
			vl_total_saida = vl_total_saida + vl_saida_acumulado
			vl_sub_total_comissao = vl_sub_total_comissao + vl_comissao_acumulado
			vl_total_comissao = vl_total_comissao + vl_comissao_acumulado
			
			x = x & "	</TR>" & chr(13)
			end if  ' if intQtdeTabelas <> 1 then
...
<tr>
	<td align="right" valign="bottom"><span class="PEDIDO">Comissão aos Vendedores Sintético (Tabela Progressiva)</span>
	<br><span class="Rc">
