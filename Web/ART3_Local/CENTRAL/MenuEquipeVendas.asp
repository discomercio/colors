...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if Not operacao_permitida(OP_CEN_CADASTRO_EQUIPE_VENDAS, s_lista_operacoes_permitidas) then 
...
var s_dest, s_op, s_id;
	...
	if (f.rb_op[0].checked) {
		s_dest="EquipeVendasEdita.asp";
		s_op=OP_INCLUI;
		s_id=f.c_novo.value;
		if (trim(f.c_novo.value)=="") {
			alert("Informe o apelido para a nova equipe de vendas!!");
			f.c_novo.focus();
			return false;
			}
		}
	...
	if (f.rb_op[1].checked) {
		...
		if (trim(f.c_cons.value)=="") {
			alert("Informe o apelido da equipe de vendas a ser consultada!!");
			f.c_cons.focus();
			return false;
			}
		}
...
<span class="T">CADASTRO DE EQUIPE DE VENDAS</span>
	...
	<tr>
		<td nowrap>
			<input type="radio" id="rb_op" name="rb_op" value="1" class="CBOX" onclick="fOP.c_novo.focus()">
				...
				<input name="c_novo" id="c_novo" type="text" maxlength="12" size="14" 
					onblur="this.value=trim(this.value);" 
					onclick="fOP.rb_op[0].click()" 
					onkeypress="this.click(); if (digitou_enter(true) && tem_info(this.value)) if (fOPConcluir(fOP)) fOP.submit(); filtra_nome_identificador();"
					><br>
			<input type="radio" id="rb_op" name="rb_op" value="2" class="CBOX" onclick="fOP.c_cons.focus()">
				...
				<input name="c_cons" id="c_cons" type="text" maxlength="12" size="14" 
					onblur="this.value=trim(this.value);"
