...
<%
'     ===========================================================================
'	  A N A L I S E C R E D I T O C A D A P R O V A U T O C O N F I R M A . A S P
'     ===========================================================================
'
'
...
'	OBTÉM DADOS DO FORMULÁRIO ANTERIOR
	dim c_valor, m_valor
	c_valor = Trim(Request.Form("c_valor"))
	m_valor = converte_numero(c_valor)
	...
	alerta = ""
	if m_valor < 0 then
		alerta="VALOR NEGATIVO NÃO É VÁLIDO."
	elseif formata_moeda(m_valor) <> c_valor then
		alerta="VALOR DIGITADO COM FORMATO INVÁLIDO."
		end if
...
'	GRAVA PARÂMETRO!!
	if alerta = "" then 
		s = "SELECT * FROM t_CONTROLE WHERE (id_nsu = '" & ID_PARAM_CAD_VL_APROV_AUTO_ANALISE_CREDITO & "')"
		r.Open s, cn
		if r.Eof then
			alerta = "PARAMETRO '" & ID_PARAM_CAD_VL_APROV_AUTO_ANALISE_CREDITO & "' NÃO FOI ENCONTRADO EM T_CONTROLE."
			end if
		...
		if alerta = "" then
			...
			If Err = 0 then 
				...
				if s_log <> "" then 
					grava_log usuario, "", "", "", OP_LOG_CAD_VL_APROV_AUTO_ANALISE_CREDITO, s_log
					end if
			else
				erro_fatal=True
