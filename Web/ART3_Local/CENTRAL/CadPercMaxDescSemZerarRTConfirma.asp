...
<%
'     ===========================================================================
'	  CadPercMaxDescSemZerarRTConfirma.asp
'     ===========================================================================
'
'
...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if Not operacao_permitida(OP_CEN_CAD_PERC_MAX_DESC_SEM_ZERAR_RT, s_lista_operacoes_permitidas) then
...
'	OBTÉM DADOS DO FORMULÁRIO ANTERIOR
	class cl_CadPercMaxDescSemZerarRT
		dim num_loja
		dim nome_loja
		dim percentual
		end class
	...
	dim vTabela
	redim vTabela(0)
	set vTabela(Ubound(vTabela)) = new cl_CadPercMaxDescSemZerarRT
	...
	'	~~~~~~~~~~~~~
		for i=Lbound(vTabela) to Ubound(vTabela)
			if Trim("" & vTabela(i).num_loja) <> "" then
				...
				if alerta = "" then
					if converte_numero(r("PercMaxDescSemZerarRT")) <> converte_numero(vTabela(i).percentual) then
						if s_log <> "" then s_log = s_log & "; "
						s_log = s_log & vTabela(i).num_loja & ": " & formata_perc(r("PercMaxDescSemZerarRT")) & " => " & formata_perc(vTabela(i).percentual)
						r("PercMaxDescSemZerarRT") = vTabela(i).percentual
						r.Update
		...
		if alerta = "" then
			if s_log <> "" then
				s_log = "Alteração do percentual de desconto máximo sem zerar a comissão da(s) seguinte(s) loja(s): " & s_log
				grava_log usuario, "", "", "", OP_LOG_PERC_MAX_DESC_SEM_ZERAR_RT, s_log
				end if
			end if
