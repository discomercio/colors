...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if Not operacao_permitida(OP_CEN_CADASTRO_GRUPO_LOJAS, s_lista_operacoes_permitidas) then 
...
var s_dest,s_op,s_grupo;
	...
	if (f.rb_op[0].checked) {
		s_dest="GrupoLojasEdita.asp";
		s_op=OP_INCLUI;
		s_grupo=f.c_novo.value;
		if (trim(f.c_novo.value)=="") {
			alert("Forneça o número para o novo grupo de lojas!!");
			f.c_novo.focus();
			return false;
			}
		}
	...
	if (f.rb_op[1].checked) {
		s_dest="GrupoLojasEdita.asp";
		s_op=OP_CONSULTA;
		s_grupo=f.c_cons.value;
		if (trim(f.c_cons.value)=="") {
			alert("Especifique o número do grupo de lojas a ser consultado!!");
			f.c_cons.focus();
			return false;
			}
		}
	...
	f.grupo_lojas_selecionado.value=s_grupo;
...
<span class="T">CADASTRO DE GRUPO DE LOJAS</span>
	...
	<tr>
		<td NOWRAP>
			<input type="RADIO" id="rb_op" name="rb_op" value="1" class="CBOX" onclick="fOP.c_novo.focus();"><span style="cursor:default" onclick="fOP.rb_op[0].click(); fOP.c_novo.focus();">Cadastrar Novo</span>&nbsp;
				<input name="c_novo" id="c_novo" type="TEXT" maxlength="2" size="6" onblur="this.value=normaliza_codigo(this.value,TAM_MIN_GRUPO_LOJAS);" onclick="fOP.rb_op[0].click()" onkeypress="this.click(); if (digitou_enter(true) && tem_info(this.value)) {this.value=normaliza_codigo(this.value,TAM_MIN_GRUPO_LOJAS); fOPConcluir(fOP);} filtra_numerico();"><br>
			<input type="RADIO" id="rb_op" name="rb_op" value="2" class="CBOX" onclick="fOP.c_cons.focus();"><span style="cursor:default" onclick="fOP.rb_op[1].click(); fOP.c_cons.focus();">Consultar</span>&nbsp;
				<input name="c_cons" id="c_cons" type="TEXT" maxlength="2" size="6" onblur="this.value=normaliza_codigo(this.value,TAM_MIN_GRUPO_LOJAS);" onclick="fOP.rb_op[1].click();" onkeypress="this.click(); if (digitou_enter(true) && tem_info(this.value)) {this.value=normaliza_codigo(this.value,TAM_MIN_GRUPO_LOJAS); fOPConcluir(fOP)} filtra_numerico();"><br>
