...
<%
'     ====================================================
'	  E S T O Q U E A T U A L I Z A E A N . A S P
'     ====================================================
'
'
...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if alerta = "" then
		...
		redim v_item(0)
		set v_item(0) = New cl_ITEM_ESTOQUE_TELA_AGIO
		...
		for i = 1 to n
			...
			if s <> "" then
				if Trim(v_item(ubound(v_item)).produto) <> "" then
					...
					set v_item(ubound(v_item)) = New cl_ITEM_ESTOQUE_TELA_AGIO
					end if
				with v_item(ubound(v_item))
					...
					.vl_BC_ICMS_ST = converte_numero(Trim(Request.Form("c_vl_BC_ICMS_ST")(i)))
					.vl_ICMS_ST = converte_numero(Trim(Request.Form("c_vl_ICMS_ST")(i)))
	...
	'	~~~~~~~~~~~~~
		cn.BeginTrans
	'	~~~~~~~~~~~~~
		if estoque_atualiza_agio(usuario, r_estoque, v_item, s_log, msg_erro) then
