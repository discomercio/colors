...
<%
'     =================================================================
'	  P E D I D O A N O T A T R A N S P C O N S I S T E . A S P
'     =================================================================
'
'
...
'	REVISADO P/ IE10
	...
	dim s, usuario, s_transportadora, lista_pedidos, v_pedido, v_aux, i, j, achou, msg_erro
...
'	=========================
	...
	if alerta = "" then
		for i = Lbound(v_pedido) to Ubound(v_pedido)
			if v_pedido(i) <> "" then
				s = "SELECT pedido, numero_loja, st_entrega, transportadora_id, id_nfe_emitente FROM t_PEDIDO WHERE (pedido='" & v_pedido(i) & "')"
				if rs.State <> 0 then rs.Close
				rs.open s, cn
				if rs.Eof then
					alerta=texto_add_br(alerta)
				...
				'	ERRO: A TRANSPORTADORA INDICADA JÁ ESTÁ ANOTADA NO PEDIDO
					if UCase(Trim("" & rs("transportadora_id"))) = UCase(s_transportadora) then
						alerta=texto_add_br(alerta)
						alerta=alerta & "Pedido " & v_pedido(i) & " já está com a transportadora especificada registrada (" & s_transportadora & ")"
						end if
					
				'	ALERTA: JÁ EXISTE UMA OUTRA TRANSPORTADORA ANOTADA NO PEDIDO
					if Trim("" & rs("transportadora_id")) <> "" then
						observacoes=texto_add_br(observacoes)
						observacoes=observacoes & "Pedido " & v_pedido(i) & " já tem registrado a transportadora: " & Trim("" & rs("transportadora_id"))
						end if
					
				'	ALERTA: STATUS DO PEDIDO JÁ É DE PEDIDO ENCERRADO
					if IsPedidoEncerrado(Trim("" & rs("st_entrega"))) then
						observacoes=texto_add_br(observacoes)
						observacoes=observacoes & "Pedido " & v_pedido(i) & " possui status: " & Ucase(x_status_entrega(Trim("" & rs("st_entrega"))))
						end if
...
function fAnotaTranspConfirma( f ) {
...
<form id="fAnotaTransp" name="fAnotaTransp" method="post" action="PedidoAnotaTranspConfirma.asp">
...
<tr>
	<td align="right" valign="bottom"><p class="PEDIDO">Anotar Transportadora no Pedido<span class="C">&nbsp;</span></p></td>
</tr>
</table>
<br>
...
<table class="Qx" cellSpacing="0">
	<tr bgColor="#FFFFFF">
		<td class="MT" NOWRAP style='background:azure;'><span class="PLTe">Transportadora&nbsp;</span></td>
	</tr>
	<tr bgColor="#FFFFFF">
		<td class="MDBE" NOWRAP><span class="PLLe" style="margin-left:3px;margin-right:3px;"><%=s_transportadora & " - " & iniciais_em_maiusculas(x_transportadora(s_transportadora))%></span></td>
	</tr>
</table>
...
<tr>
	...
	<a name="bCONFIRMA" id="bCONFIRMA" href="javascript:fAnotaTranspConfirma(fAnotaTransp)" title="confirma a anotação da transportadora no(s) pedido(s)">
