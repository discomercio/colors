...
' REL_BRASPAG_AF_REVIEW_STATUS_MONTA_ITENS_SELECT
'
function rel_braspag_af_review_status_monta_itens_select(byval id_default)
dim ha_default
dim listaOpcoes
dim i, x, strResp
	listaOpcoes = Array(COD_REL_BRASPAG_AF_REVIEW__REVISAO_MANUAL_PENDENTE, _
						COD_REL_BRASPAG_AF_REVIEW__REVISAO_MANUAL_TRATADA_ACCEPT, _
						COD_REL_BRASPAG_AF_REVIEW__REVISAO_MANUAL_TRATADA_REJECT, _
						COD_REL_BRASPAG_AF_REVIEW__APROVADO_AUTOMATICAMENTE, _
						COD_REL_BRASPAG_AF_REVIEW__REJEITADO_AUTOMATICAMENTE)
	...
	for i = Lbound(listaOpcoes) to Ubound(listaOpcoes)
		...
		if x <> "" then
			...
			strResp = strResp & " value='" & x & "'>" & _
								descricao_cod_rel_braspag_af_review(x) & _
								"</option>" & chr(13)
			end if
		next
	...
	rel_braspag_af_review_status_monta_itens_select = strResp
end function



' __________________________________________________________
' REL_BRASPAG_AF_REVIEW_BANDEIRA_MONTA_ITENS_SELECT
'
function rel_braspag_af_review_bandeira_monta_itens_select(byval id_default)
dim ha_default
dim listaOpcoes
dim i, x, strResp
	...
	rel_braspag_af_review_bandeira_monta_itens_select = strResp
end function
%>
...
<script type="text/javascript">
	$(function() {
		$("#c_dt_transacao_inicio").hUtilUI('datepicker_filtro_inicial');
		$("#c_dt_transacao_termino").hUtilUI('datepicker_filtro_final');
		$("#c_dt_tratado_inicio").hUtilUI('datepicker_filtro_inicial');
		$("#c_dt_tratado_termino").hUtilUI('datepicker_filtro_final');
	});
</script>
...
var s_de, s_ate;
var blnHaRestricaoPorPeriodo;

	blnHaRestricaoPorPeriodo = false;
	if ((trim(f.c_dt_transacao_inicio.value) != "") && (trim(f.c_dt_transacao_termino.value) != "")) {
		if (!isDate(f.c_dt_transacao_inicio)) {
			alert("Data de início inválida!!");
			f.c_dt_transacao_inicio.focus();
			return;
		}
		if (!isDate(f.c_dt_transacao_termino)) {
			alert("Data de término inválida!!");
			f.c_dt_transacao_termino.focus();
			return;
		}
		s_de = trim(f.c_dt_transacao_inicio.value);
		s_ate = trim(f.c_dt_transacao_termino.value);
		if ((s_de != "") && (s_ate != "")) {
		...
		blnHaRestricaoPorPeriodo = true;
	}
	
	if ((trim(f.c_dt_tratado_inicio.value) != "") && (trim(f.c_dt_tratado_termino.value) != "")) {
		if (!isDate(f.c_dt_tratado_inicio)) {
			alert("Data de início inválida!!");
			f.c_dt_tratado_inicio.focus();
			return;
		}
		if (!isDate(f.c_dt_tratado_termino)) {
			alert("Data de término inválida!!");
			f.c_dt_tratado_termino.focus();
			return;
		}
		s_de = trim(f.c_dt_tratado_inicio.value);
		s_ate = trim(f.c_dt_tratado_termino.value);
		if ((s_de != "") && (s_ate != "")) {
			s_de = retorna_so_digitos(formata_ddmmyyyy_yyyymmdd(s_de));
			s_ate = retorna_so_digitos(formata_ddmmyyyy_yyyymmdd(s_ate));
			if (s_de > s_ate) {
	...
	if (!blnHaRestricaoPorPeriodo) {
		alert("É necessário informar algum período de datas para restringir o resultado da consulta!!");
		return;
	}
...
<body onload="fFILTRO.c_dt_transacao_inicio.focus();">
<center>

<form id="fFILTRO" name="fFILTRO" method="post" action="RelBraspagAfReviewExec.asp">
...
<!--  PERÍODO DA TRANSAÇÃO  -->
	<tr>
		<td class="ME MD MC" align="left" nowrap><span class="PLTe">PERÍODO: TRANSAÇÃO EFETUADA</span></td>
	</tr>
	<tr bgcolor="#FFFFFF" nowrap>
		<td class="ME MD MB" align="left">
			...
			<tr bgcolor="#FFFFFF">
				<td align="left">
					<input class="Cc" maxlength="10" style="width:90px;" name="c_dt_transacao_inicio" id="c_dt_transacao_inicio" onblur="if (!isDate(this)) {alert('Data de início inválida!'); this.focus();}" onkeypress="if (digitou_enter(true)) fFILTRO.c_dt_transacao_termino.focus(); filtra_data();"
						<%if c_dt_transacao_inicio <> "" then Response.Write " value=" & chr(34) & c_dt_transacao_inicio & chr(34)%>
						/>&nbsp;
					<span class="C">&nbsp;até&nbsp;</span>&nbsp;
					<input class="Cc" maxlength="10" style="width:90px;" name="c_dt_transacao_termino" id="c_dt_transacao_termino" onblur="if (!isDate(this)) {alert('Data de término inválida!'); this.focus();}" onkeypress="if (digitou_enter(true)) bCONFIRMA.focus(); filtra_data();"
						<%if c_dt_transacao_termino <> "" then Response.Write " value=" & chr(34) & c_dt_transacao_termino & chr(34)%>
						/>
				</td>
			</tr>
			</table>
...
<!--  PERÍODO DO TRATAMENTO  -->
	<tr>
		<td class="ME MD" align="left" nowrap><span class="PLTe">PERÍODO: REVISÃO TRATADA</span></td>
	</tr>
	<tr bgcolor="#FFFFFF" nowrap>
		<td class="ME MD MB" align="left">
			<table cellspacing="0" cellpadding="0" style="margin:0px 20px 6px 10px;">
			<tr bgcolor="#FFFFFF">
				<td align="left">
					<input class="Cc" maxlength="10" style="width:90px;" name="c_dt_tratado_inicio" id="c_dt_tratado_inicio" onblur="if (!isDate(this)) {alert('Data de início inválida!'); this.focus();}" onkeypress="if (digitou_enter(true)) fFILTRO.c_dt_tratado_termino.focus(); filtra_data();"
						<%if c_dt_tratado_inicio <> "" then Response.Write " value=" & chr(34) & c_dt_tratado_inicio & chr(34)%>
						/>&nbsp;
					<span class="C">&nbsp;até&nbsp;</span>&nbsp;
					<input class="Cc" maxlength="10" style="width:90px;" name="c_dt_tratado_termino" id="c_dt_tratado_termino" onblur="if (!isDate(this)) {alert('Data de término inválida!'); this.focus();}" onkeypress="if (digitou_enter(true)) bCONFIRMA.focus(); filtra_data();"
						<%if c_dt_tratado_termino <> "" then Response.Write " value=" & chr(34) & c_dt_tratado_termino & chr(34)%>
						/>
				</td>
			</tr>
			</table>
...
<!--  RESULTADO DA TRANSAÇÃO  -->
	...
	<tr bgcolor="#FFFFFF" nowrap>
		<td class="ME MB MD" align="left">
			...
			<%=rel_braspag_af_review_status_monta_itens_select(c_resultado_transacao)%>
			</select>
		</td>
	</tr>
...
<!--  BANDEIRA  -->
	...
	<tr bgcolor="#FFFFFF" nowrap>
		<td class="ME MB MD" align="left">
			...
			<%=rel_braspag_af_review_bandeira_monta_itens_select(c_bandeira)%>
			</select>
		</td>
	</tr>
