...
'	=========================
	...
	'	Realiza a verificação após obter o lock do registro de controle p/ assegurar que não haverá problemas de acesso concorrente
		...
		if alerta = "" then
			...
			if Not grava_bloco_notas_pedido(id_pedido_filhote, ID_USUARIO_SISTEMA, "", COD_NIVEL_ACESSO_BLOCO_NOTAS_PEDIDO__PUBLICO, sBlocoNotasMsg, COD_TIPO_MSG_BLOCO_NOTAS_PEDIDO__AUTOMATICA_SPLIT_MANUAL, msg_erro) then
		...
		'	PEDIDO-BASE: ATUALIZA STATUS DE ENTREGA
			...
			s = "UPDATE tPed SET" & _
				...
				" WHERE" & _
					" (tPed.pedido = '" & id_pedido_filhote & "')" & _
