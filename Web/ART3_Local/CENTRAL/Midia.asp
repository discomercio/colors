...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	if Not operacao_permitida(OP_CEN_CADASTRO_VEICULOS_MIDIA, s_lista_operacoes_permitidas) then 
...
var s_dest, s_op, s_midia;
	...
	if (f.rb_op[0].checked) {
		s_dest="midiaedita.asp";
		s_op=OP_INCLUI;
		s_midia=f.c_novo.value;
		if (trim(f.c_novo.value)=="") {
			alert("Forneça o número para o novo veículo de mídia!!");
			f.c_novo.focus();
			return false;
			}
		}
	...
	if (f.rb_op[1].checked) {
		s_dest="midiaedita.asp";
		s_op=OP_CONSULTA;
		s_midia=f.c_cons.value;
		if (trim(f.c_cons.value)=="") {
			alert("Especifique o número do veículo de mídia a ser consultado!!");
			f.c_cons.focus();
			return false;
			}
		}
	...
	f.midia_selecionada.value=s_midia;
...
<span class="T">CADASTRO DE VEÍCULOS DE MÍDIA</span>
	...
	<tr>
		<td nowrap>
			<input type="radio" id="rb_op" name="rb_op" value="1" class="CBOX" onclick="fOP.c_novo.focus();"><span style="cursor:default" onclick="fOP.rb_op[0].click(); fOP.c_novo.focus();">Cadastrar Novo</span>&nbsp;
				<input name="c_novo" id="c_novo" type="text" maxlength="3" size="6" onblur="this.value=normaliza_codigo(this.value,TAM_MIN_MIDIA);" onclick="fOP.rb_op[0].click()" onkeypress="this.click(); if (digitou_enter(true) && tem_info(this.value)) if (fOPConcluir(fOP)) fOP.submit(); filtra_numerico();"><br>
			<input type="radio" id="rb_op" name="rb_op" value="2" class="CBOX" onclick="fOP.c_cons.focus();"><span style="cursor:default" onclick="fOP.rb_op[1].click(); fOP.c_cons.focus();">Consultar</span>&nbsp;
				<input name="c_cons" id="c_cons" type="text" maxlength="3" size="6" onblur="this.value=normaliza_codigo(this.value,TAM_MIN_MIDIA);" onclick="fOP.rb_op[1].click();" onkeypress="this.click(); if (digitou_enter(true) && tem_info(this.value)) if (fOPConcluir(fOP)) fOP.submit(); filtra_numerico();"><br>
