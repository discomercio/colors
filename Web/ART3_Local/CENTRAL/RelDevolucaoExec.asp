...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_CEN_REL_DEVOLUCAO_PRODUTOS, s_lista_operacoes_permitidas) then 
		cn.Close
...
'	CRITÉRIO: LOJA
	...
	elseif rb_loja = "FAIXA" then
		...
		if c_loja_de <> "" then
			if s <> "" then s = s & " AND"
			s = s & " (CONVERT(smallint, t_PEDIDO.loja) >= " & c_loja_de & ")"
			end if

		if c_loja_ate <> "" then
			if s <> "" then s = s & " AND"
			s = s & " (CONVERT(smallint, t_PEDIDO.loja) <= " & c_loja_ate & ")"
			end if
...
<% 
	...
	s_filtro = s_filtro & "<tr><td align='right' valign='top' NOWRAP>" & _
			   "<p class='N'>Lojas:&nbsp;</p></td><td valign='top'>" & _
			   "<p class='N'>" & s & "</p></td></tr>"
