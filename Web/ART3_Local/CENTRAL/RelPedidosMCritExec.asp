...
dim s, s_aux, s_resp
	...
	s_aux = ""
	if ckb_obs2_nao_preenchido <> "" then s_aux = "OBS II não preenchido"
	if s_aux<>"" then
		if s <> "" then s = s & ", "
		s = s & s_aux
		end if
...
<% 
	...
	if ckb_visanet <> "" then
		s_filtro = s_filtro & _
					...
					"		<td valign='top' width='99%'><p class='N'>somente pedidos pagos usando cartão de crédito</p></td>" & chr(13) & _
					"	</tr>" & chr(13)
		end if
