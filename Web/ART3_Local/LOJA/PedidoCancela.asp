...
'	VERIFICA SE A NFe JÁ FOI EMITIDA
	...
	if alerta = "" then
		if Not operacao_permitida(OP_LJA_CANCELAR_PEDIDO_COM_NFE_EMITIDA, s_lista_operacoes_permitidas) then
	...
	'	~~~~~~~~~~~~~
		...
		if rs.EOF then
			...
			elseif Trim("" & rs("obs_2")) <> "" then
				alerta="Pedido " & pedido_selecionado & " não pode ser cancelado enquanto o campo 'Nº Nota Fiscal' estiver preenchido."
			...
			'	========= CANCELA LISTA DE PRODUTOS VENDIDOS SEM PRESENÇA NO ESTOQUE.
				...
				grava_log usuario, loja, pedido_selecionado, "", OP_LOG_PEDIDO_CANCELA, s_log
