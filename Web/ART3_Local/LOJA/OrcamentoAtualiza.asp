...
'	=========================
	...
	dim r_orcamento, v_item_bd
	if Not le_orcamento(orcamento_selecionado, r_orcamento, msg_erro) then 
		alerta = msg_erro
	else
		if Not le_orcamento_item(orcamento_selecionado, v_item_bd, msg_erro) then 
            alerta = msg_erro
        else
...
'	FORMA DE PAGAMENTO (NOVA VERSÃO)
	...
	dim EdicaoItemOrcamentoLiberada, blnEdicaoItemOrcamentoLiberada
	EdicaoItemOrcamentoLiberada = Trim(Request.Form("EdicaoItemOrcamentoLiberada"))
	blnEdicaoItemOrcamentoLiberada = False
	if EdicaoItemOrcamentoLiberada = "S" then blnEdicaoItemOrcamentoLiberada = True
	...
	for i = 1 to n
		...
		if s <> "" then
			...
			with v_item(ubound(v_item))
				...
				if (r_orcamento.permite_RA_status = 1) Or (blnTemRA And (r_orcamento.st_violado_permite_RA_status = 1)) then
...
'	FORMA DE PAGAMENTO (NOVA VERSÃO)
	if alerta = "" then
		if (versao_forma_pagamento = "2") And (Not blnFormaPagtoBloqueado) then
...
'	O PRÉ-PEDIDO FOI CADASTRADO JÁ DENTRO DA POLÍTICA DE PERCENTUAL DE CUSTO FINANCEIRO POR FORNECEDOR?
	...
	if c_FlagEndEntregaEditavel = "S" then
		if alerta = "" then
            if (EndEtg_endereco<>r_orcamento.EndEtg_endereco) Or (EndEtg_bairro<>r_orcamento.EndEtg_bairro) Or (EndEtg_cidade<>r_orcamento.EndEtg_cidade) Or (EndEtg_uf<>r_orcamento.EndEtg_uf) Or (EndEtg_cep<>r_orcamento.EndEtg_cep) Or (EndEtg_obs<>r_orcamento.EndEtg_cod_justificativa) then
                blnEndEtg_obs = true 
                end if
            'na memorizacao de endereços ligada, sempre verificamos
            if r_orcamento.st_memorizacao_completa_enderecos = 1 and blnUsarMemorizacaoCompletaEnderecos then
                blnEndEtg_obs = true 
                end if
			...
			if r_orcamento.st_memorizacao_completa_enderecos = 1 or r_orcamento.st_memorizacao_completa_enderecos = 9 then
				...
				if  eh_cpf then
					...
					if cliente__ddd_com_2 <> "" and cliente__tel_com_2 = "" or cliente__ddd_com <> "" and cliente__ramal_com_2 <> "" and cliente__tel_com_2 = "" then 
						alerta = "Telefone inválido!!!"
					end if
        ...
        if alerta = "" and blnEndEtgComDados and r_orcamento.st_memorizacao_completa_enderecos = 1 and blnUsarMemorizacaoCompletaEnderecos and Not eh_cpf then
...
'	=======================
	...
	'	~~~~~~~~~~~~~
		...
		if alerta = "" then
			...
			elseif rs.EOF then
				alerta = "Pré-Pedido " & orcamento_selecionado & " não foi encontrado."
			else
			...
			'	Forma de Pagamento (nova versão)
				...
				if (Not blnFormaPagtoBloqueado) then rs("forma_pagto") = s_forma_pagto
				
				if (versao_forma_pagamento = "1") And (Not blnFormaPagtoBloqueado) then
		...
		if alerta = "" then
		'	O PRÉ FOI CADASTRADO JÁ DENTRO DA POLÍTICA DE PERCENTUAL DE CUSTO FINANCEIRO POR FORNECEDOR?
			if c_custoFinancFornecTipoParcelamentoOriginal <> "" then
					...
					if (c_custoFinancFornecTipoParcelamentoOriginal <> c_custoFinancFornecTipoParcelamento) Or _
					   (c_custoFinancFornecQtdeParcelasOriginal <> c_custoFinancFornecQtdeParcelas) then
						...
						if alerta = "" then
							...
							if rs.Eof then
								alerta=texto_add_br(alerta)
								alerta=alerta & "Falha ao localizar o registro do pré-pedido " & orcamento_selecionado
							else
				...
				for i=Lbound(v_item) to Ubound(v_item)
					with v_item(i)
						if Trim(.produto)<>"" then
							...
							elseif rs.EOF then
								alerta=texto_add_br(alerta)
								alerta=alerta & "Item do Pré-Pedido do produto " & Trim(.produto) & " (fabricante " & Trim(.fabricante) & ") não foi encontrado."
							else
								editou_item = False
							'	EDITOU "OBSERVAÇÕES"?
		...
		'	GRAVA O LOG!!
			if s_log <> "" then grava_log usuario, loja, orcamento_selecionado, "", OP_LOG_ORCAMENTO_ALTERACAO, s_log
		'	~~~~~~~~~~~~~~
			cn.CommitTrans
		'	~~~~~~~~~~~~~~
			if Err=0 then 
