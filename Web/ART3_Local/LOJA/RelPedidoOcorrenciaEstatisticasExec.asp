...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	dim c_transportadora, c_vendedor, c_indicador, c_uf,rb_status
	...
	rb_status = Ucase(Trim(Request.Form("rb_status")))
...
'	MONTA SQL
    ...
    if rb_status = "ABERTA" then
		s_sql = s_sql & " (tPO.finalizado_status = 0)"
	elseif rb_status = "EM_ANDAMENTO" then
		s_sql = s_sql & " (tPO.finalizado_status = 0)"
    elseif rb_status = "FINALIZADO" then
        s_sql = s_sql & " (tPO.finalizado_status <> 0)"
    elseif rb_status = "TODAS_AS_OPCOES" then
        s_sql = s_sql &  " ((tPO.finalizado_status = 0) Or (tPO.finalizado_status <> 0)) "
	end if
	...
	if IsDate(c_dt_cad_ocorrencia_termino) then
		s_sql = s_sql & _
					" AND (tPO.dt_cadastro < " & bd_formata_data(StrToDate(c_dt_cad_ocorrencia_termino)+1) & ")"
		end if
...
'	LOJA
	s_where_loja = ""
	if loja <> "" then
			s_where_loja = s_where_loja & " (tP.numero_loja = " & loja & ")"
	end if
