...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_LJA_REL_GERENCIAL_DE_VENDAS, s_lista_operacoes_permitidas) then 
		cn.Close
	...
	if (rb_saida = COD_SAIDA_REL_INDICADOR) Or (rb_saida = COD_SAIDA_REL_INDICADOR_UF) then
		c_forma_como_conheceu_codigo = Trim(Request.Form("c_forma_como_conheceu_codigo"))
		end if
...
dim intLargIndicador, intLargUF, intLargColPerc, intLargColMonetario
dim intLargApelidoVendedor, intLargApelidoIndicador, intLargRazaoSocial, intLargMunicipio
dim r
dim s, s_aux, s_where, s_where_venda, s_where_devolucao, s_where_loja, s_cor
...
'	CRITÉRIOS COMUNS
    ...
    if c_pedido_origem <> "" then
        if s_where <> "" then s_where = s_where & " AND"
        s_where = s_where & " (t_PEDIDO.marketplace_codigo_origem = '" & c_pedido_origem & "')"
    end if
    ...
    if c_grupo_pedido_origem <> "" then
        c_grupo_pedido_origem_aux = ""
        s = "SELECT codigo FROM t_CODIGO_DESCRICAO WHERE (codigo_pai = '" & c_grupo_pedido_origem & "') AND grupo='PedidoECommerce_Origem'"
        if rs.State <> 0 then rs.Close
	    rs.open s, cn
		if rs.Eof then
            ...
            do while Not rs.Eof
                if c_grupo_pedido_origem_aux <> "" then c_grupo_pedido_origem_aux = c_grupo_pedido_origem_aux & ", "
                c_grupo_pedido_origem_aux = c_grupo_pedido_origem_aux & "'" & rs("codigo") & "'"      
                rs.MoveNext
            loop
        end if
        if s_where <> "" then s_where = s_where & " AND"
	...
	if (rb_saida = COD_SAIDA_REL_INDICADOR) Or (rb_saida = COD_SAIDA_REL_INDICADOR_UF) then
		if c_forma_como_conheceu_codigo <> "" then
			if s_where <> "" then s_where = s_where & " AND"
			s_where = s_where & " (t_ORCAMENTISTA_E_INDICADOR.forma_como_conheceu_codigo = '" & c_forma_como_conheceu_codigo & "')"
			end if
		end if
...
'	CRITÉRIOS PARA PEDIDOS DE VENDA NORMAIS
	...
	if rb_periodo = COD_CONSULTA_POR_PERIODO_ENTREGA then
		...
		if ckb_colocado_mesmo_periodo = "1" then
			if s_where_venda <> "" then s_where_venda = s_where_venda & " AND" 
			s_where_venda = s_where_venda & " (t_PEDIDO.data >= " & bd_formata_data(StrToDate(c_dt_entregue_inicio)) & ")"
			s_where_venda = s_where_venda & " AND (t_PEDIDO.data < " & bd_formata_data(StrToDate(c_dt_entregue_termino)+1) & ")"
			end if
		end if
...
'	MONTA SQL DE CONSULTA
	...
	'	QUAL É O CAMPO DE SAÍDA SELECIONADO?
		...
		elseif rb_saida = COD_SAIDA_REL_INDICADOR then
			strSqlCampoSaida =	"t_PEDIDO__BASE.indicador, " & _
								"t_ORCAMENTISTA_E_INDICADOR.razao_social_nome_iniciais_em_maiusculas, " & _
								"t_PEDIDO__BASE.vendedor, " & _
								"t_ORCAMENTISTA_E_INDICADOR.cidade, " & _
  ...
  ' CABEÇALHO
	...
	intLargVendedor = 270
	intLargApelidoVendedor = 80
	intLargIndicador = 270
	intLargApelidoIndicador = 80
	intLargRazaoSocial = 190
	intLargMunicipio = 120
	intLargUF = 30
	...
	elseif rb_saida = COD_SAIDA_REL_INDICADOR then
		...
		cab = cab & _
			...
			"		<td class='MTD' style='width:" & CStr(intLargRazaoSocial) & "px' align='left' valign='bottom' nowrap><span class='Rc'>Razão Social</span></td>" & chr(13) & _
...
'	PERCORRE OS REGISTROS OBTIDOS DO BD E CONSOLIDA OS VALORES
	do while Not r.Eof
		...
		elseif rb_saida = COD_SAIDA_REL_INDICADOR then
			strItemAtual = Trim("" & r("indicador")) & "|" & Trim("" & r("vendedor")) & "|" & Trim("" & r("razao_social_nome_iniciais_em_maiusculas")) & "|" & Trim("" & r("cidade")) & "|" & Trim("" & r("uf"))
		elseif rb_saida = COD_SAIDA_REL_UF then
...
'			======================
			...
			'> INDICADOR
				...
				s = Trim("" & v(idxAux))
				if s = "" then s = "&nbsp;"
				x = x & "		<td class='MDTE' style='width:" & CStr(intLargApelidoIndicador) & "px;' align='left' valign='bottom'><span class='Cn' style='color:" & s_cor & ";'>" & s & "</span></td>" & chr(13)
			'> VENDEDOR
				idxAux = idxAux + 1
				s = Trim("" & v(idxAux))
				if s = "" then s = "&nbsp;"
				x = x & "		<td class='MTD' style='width:" & CStr(intLargApelidoVendedor) & "px;' align='left' valign='bottom'><span class='Cn' style='color:" & s_cor & ";'>" & s & "</span></td>" & chr(13)
			'> RAZÃO SOCIAL
				idxAux = idxAux + 1
				s = Trim("" & v(idxAux))
				if s = "" then s = "&nbsp;"
				x = x & "		<td class='MTD' style='width:" & CStr(intLargRazaoSocial) & "px;' align='left' valign='bottom'><span class='Cn' style='color:" & s_cor & ";'>" & s & "</span></td>" & chr(13)
			'> MUNICÍPIO
				idxAux = idxAux + 1
				s = Trim("" & v(idxAux))
				if s = "" then s = "&nbsp;"
				x = x & "		<td class='MTD' style='width:" & CStr(intLargMunicipio) & "px;' align='left' valign='bottom'><span class='Cn' style='color:" & s_cor & ";'>" & s & "</span></td>" & chr(13)
			'> UF
				idxAux = idxAux + 1
				s = UCase(Trim("" & v(idxAux)))
				if s = "" then s = "&nbsp;"
					...
					'   %MARGEM CONTRIBUIÇÃO
						...
						x = x & "	<tr nowrap style='background: #FFFFDD'>" & chr(13) & _
								...
								"TOTAL:</span></td>" & chr(13) & _
                                 "		<td class='MTB' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_inteiro(intQtdeTotalUF) & "</span></td>" & chr(13) & _
				...
				if strAuxUFAnterior <> strAuxUF then
					...
					'	% MARGEM CONTRIB BRUTA
						...
						x = x & "	<tr nowrap style='background: #FFFFDD'>" & chr(13) & _
								...
								"		<td class='MTBE' align='right' nowrap><span class='Cd' style='color:" & s_cor & ";'>" & _
								"TOTAL:</span></td>" & chr(13) & _
                                "		<td class='MTB' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_inteiro(intQtdeTotalCidUF) & "</span></td>" & chr(13) & _
								"		<td class='MTB' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_moeda(vl_venda_UF) & "</span></td>" & chr(13) & _
								"		<td class='MTBD' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_perc(percFatVenda_UF) & "%" & "</span></td>" & chr(13) & _
								"		<td class='MTB' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_moeda(vl_lista_UF) & "</span></td>" & chr(13) & _
								"		<td class='MTB' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_moeda(vl_desconto_UF) & "</span></td>" & chr(13) & _
	...
	'	ÚLTIMO SUBTOTAL NO RELATÓRIO INDICADOR/UF E CIDADE/UF
        ...
        '  QTDE
            if rb_saida = COD_SAIDA_REL_INDICADOR_UF then
               intQtdeTotal = intQtdeTotalUf       
                ...
                intQtdeTotal = intQtdeTotalCidUf
            end if
			s_cor="black"
			if vl_total_venda < 0 then s_cor="red"
			x = x & "	</tr>" & chr(13)
		...
		'	CRIANDO ESPAÇOS EM BRANCO PARA ALINHAR A TABELA COM O TOTAL DE TODOS OS ESTADOS
			...
			x = x & "	<tr nowrap style='background:azure'>" & chr(13) & _
					"		<td valign='bottom' style='background:white;' align='left' nowrap>" & strEspacos & "</td>" & chr(13) & _
...
'		====================
		if rb_saida = COD_SAIDA_REL_PRODUTO then
			...
			x = x & "	<tr nowrap style='background: #FFFFDD'>" & chr(13) & _
					...
					"		<td class='MTBD' align='right'><span class='Cd' style='color:" & s_cor & ";'>" & formata_moeda(vl_total_RA_liquido) & "</span></td>" & chr(13) 
					if rb_periodo = COD_CONSULTA_POR_PERIODO_ENTREGA then
					    x = x & _
...
<input type="hidden" name="c_forma_como_conheceu_codigo" id="c_forma_como_conheceu_codigo" value="<%=c_forma_como_conheceu_codigo%>">
