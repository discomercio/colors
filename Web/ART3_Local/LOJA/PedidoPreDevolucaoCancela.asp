...
'	=========================
	...
	'	~~~~~~~~~~~~~
		...
		if alerta = "" then
			...
			if s_log <> "" then grava_log usuario, loja, pedido_selecionado, "", "PED DEVOL CANCELA", s_log
				...
				if s_email_vendedor <> "" then
					...
					if UCase(usuario) <> UCase(r_pedido.vendedor) then
						if (s_unidade_negocio = COD_UNIDADE_NEGOCIO_LOJA__BS) Or (s_unidade_negocio = COD_UNIDADE_NEGOCIO_LOJA__VRF) then
							obtem_descricao_status_devolucao COD_ST_PEDIDO_DEVOLUCAO__CANCELADA, s_descricao_status_devolucao, s_cor_status_devolucao
