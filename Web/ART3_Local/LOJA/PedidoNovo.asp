...
'	REVISADO P/ IE10
	...
	if c_FlagCadSemiAutoPedMagento_FluxoOtimizado = "1" then
		...
		EndCob_email_boleto = r_cliente.email_boleto
...
'	CONSISTÊNCIAS P/ EMISSÃO DE NFe
	...
	'	Validação do DDD dos telefones
		if EndCob_tipo_pessoa = ID_PF then
			if (EndCob_tel_res <> "") And (Len(EndCob_ddd_res) < 2) then
				alerta=texto_add_br(alerta)
				alerta=alerta & "DDD inválido para o telefone (endereço de cobrança): " & EndCob_tel_res
				end if

			if (EndCob_tel_cel <> "") And (Len(EndCob_ddd_cel) < 2) then
				alerta=texto_add_br(alerta)
				alerta=alerta & "DDD inválido para o telefone (endereço de cobrança): " & EndCob_tel_cel
				end if

			if (EndCob_tel_com <> "") And (Len(EndCob_ddd_com) < 2) then
				alerta=texto_add_br(alerta)
				alerta=alerta & "DDD inválido para o telefone (endereço de cobrança): " & EndCob_tel_com
				end if
		else
			if (EndCob_tel_com <> "") And (Len(EndCob_ddd_com) < 2) then
				alerta=texto_add_br(alerta)
			...
			if (EndCob_tel_com_2 <> "") And (Len(EndCob_ddd_com_2) < 2) then
				alerta=texto_add_br(alerta)
				alerta=alerta & "DDD inválido para o telefone (endereço de cobrança): " & EndCob_tel_com_2
				end if
			end if 'if EndCob_tipo_pessoa = ID_PF
