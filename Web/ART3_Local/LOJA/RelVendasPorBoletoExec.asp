...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	...
	dim s_nome_vendedor, s_nome_indicador, s_nome_analista
	...
	if alerta = "" then
		if rb_saida = COD_SAIDA_REL_INDICADORES_DO_VENDEDOR then
			...
			if c_indicadores_do_vendedor <> "" then
				...
				if rs.Eof then 
					alerta=texto_add_br(alerta)
					alerta = "VENDEDOR " & c_indicadores_do_vendedor & " NÃO ESTÁ CADASTRADO."
				else
					s_nome_vendedor = Ucase(Trim("" & rs("nome")))
					end if
				end if
	...
	if alerta = "" then
		if rb_saida = COD_SAIDA_REL_ANALISTA_CREDITO then
			if c_analista <> "" then
				...
				if rs.Eof then 
					alerta=texto_add_br(alerta)
					alerta = "ANALISTA " & c_analista & " NÃO ESTÁ CADASTRADO."
				else
...
'	SOMENTE PEDIDOS DESTA LOJA
	s = " (t_PEDIDO__X.numero_loja = " & loja & ")"
	if s_where <> "" then s_where = s_where & " AND"
	s_where = s_where & " (" & s & ")"
