...
<%
'     =================================================================
'	  RelPedidosPendentesCartaoExec.asp
'     =================================================================
'
'
...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_LJA_REL_PEDIDOS_PENDENTES_CARTAO, s_lista_operacoes_permitidas) then 
		cn.Close
...
'	CRITÉRIO: ANÁLISE DE CRÉDITO
	s = " analise_credito = " & COD_AN_CREDITO_ST_INICIAL
...
'	PEDIDO C/ PAGAMENTO INTEGRAL NO CARTÃO E QUE DEVE PASSAR PELA ANÁLISE DE ENDEREÇO: O CLIENTE PODE TER FEITO O PAGAMENTO ANTES DA ANÁLISE DE CRÉDITO TER SIDO REALIZADA, PORTANTO, A RETIRADA DESSE PEDIDO DESTA LISTAGEM SERÁ ATRAVÉS DO STATUS DE PAGAMENTO
	s = "(t_PEDIDO.st_pagto <> '" & ST_PAGTO_PAGO & "')"
	if s_where <> "" then s_where = s_where & " AND"
	s_where = s_where & " (" & s & ")"
...
'	MONTA CLÁUSULA FROM
	...
	s_sql = "SELECT t_PEDIDO.loja, t_PEDIDO.numero_loja," & _
			...
			" t_PEDIDO.pce_forma_pagto_entrada, t_PEDIDO.pce_forma_pagto_prestacao, t_PEDIDO.pce_entrada_valor, t_PEDIDO.pce_prestacao_qtde," & _
			" t_PEDIDO.pce_prestacao_valor, t_PEDIDO.pce_prestacao_periodo, t_PEDIDO.pse_forma_pagto_prim_prest," & _
			" t_PEDIDO.pse_forma_pagto_demais_prest, t_PEDIDO.pse_prim_prest_valor, t_PEDIDO.pse_prim_prest_apos," & _
			" t_PEDIDO.pse_demais_prest_qtde, t_PEDIDO.pse_demais_prest_valor, t_PEDIDO.pse_demais_prest_periodo" & _
			s_from & _
			s_where
  ...
  ' CABEÇALHO
	...
	do while Not r.Eof
		if is_forma_pagto_somente_cartao(r) then
		'	MUDOU DE VENDEDOR?
		...
		'> TOTALIZAÇÃO DE VALORES
			...
			end if ' if is_forma_pagto_somente_cartao(r)
...
<tr>
	<td align="right" valign="bottom"><span class="PEDIDO">Relatório de Pedidos Pendentes Cartão de Crédito</span>
	<br><span class="Rc">
