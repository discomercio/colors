...
'	REVISADO P/ IE10
	...
	dim s, s_log, usuario, loja, qtde_senhas, intQtdeRegsAfetados
	usuario = Trim(Session("usuario_atual"))
	loja = Trim(Session("loja_atual"))
	If (usuario = "") then Response.Redirect("aviso.asp?id=" & ERR_SESSAO) 
	If (loja = "") then Response.Redirect("aviso.asp?id=" & ERR_SESSAO) 

	dim cliente_selecionado, c_loja
...
'	=========================
	...
	if alerta = "" then
		...
		if qtde_senhas <= 0 then
			alerta = "Não há senhas em aberto para este cliente nesta loja."
			end if
		end if
	...
	'	INFORMAÇÕES PARA O LOG
		s_log = "Cancelamento das senhas de autorização para desconto superior concedidas para o cliente " & _
				cnpj_cpf_formata(r_cliente.cnpj_cpf) & " na loja " & c_loja
	'	~~~~~~~~~~~~~
		cn.BeginTrans
	'	~~~~~~~~~~~~~
		...
		grava_log usuario, c_loja, "", cliente_selecionado, OP_LOG_DESC_SUP_CANCELA_NA_LOJA, s_log
	'	~~~~~~~~~~~~~~
		cn.CommitTrans
	'	~~~~~~~~~~~~~~
		if Err=0 then 
