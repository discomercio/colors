...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_LJA_REL_PEDIDO_CHAMADO, s_lista_operacoes_permitidas) then 
		cn.Close
	...
	'	PARÂMETRO VEM PELA QUERYSTRING
        ...
        blnHasDepto = CBool(Request("blnHasDepto"))
...
dim r
dim cab_table, cab
dim qtde_chamado_aberto, qtde_chamado_em_andamento
	...
	s_sql = _
	   ...
	   " WHERE" & _
			" (finalizado_status = 0)" & _
            " AND (tP.loja = '" & loja & "')"

    if c_depto <> "" then
        s_sql = s_sql & " AND (tPC.id_depto = '" & c_depto & "')"
    else
        if Not operacao_permitida(OP_LJA_PEDIDO_CHAMADO_CONSULTA_CHAMADOS_TODOS_DEPTOS, s_lista_operacoes_permitidas) then
	...
	if rb_status = "ABERTO" then
		s_sql = "SELECT * FROM (" & s_sql & ") t WHERE (qtde_msg_rx = 0) ORDER BY dt_hr_cadastro, id"
	...
	'> MOTIVO FINALIZAÇÃO
		x = x & "	<TR style='display:none;' id='TR_MOTIVO_FINALIZACAO_" & Cstr(intQtdeChamados) & "'>" & chr(13) & _
				...
				"				<TR>" & chr(13) & _
				"					<TD>" & chr(13) & _
										"<select id='c_motivo_finalizacao_" & Cstr(intQtdeChamados) & "' name='c_motivo_finalizacao_" & Cstr(intQtdeChamados) & "' style='width:600px;margin-left:4pt;margin-top:4pt;margin-bottom:4pt;' onkeyup='if (window.event.keyCode==KEYCODE_DELETE) this.options[0].selected=true;'>" & chr(13) & _
										motivo_finalizacao_chamado_monta_itens_select(Trim("" & r("cod_depto"))) & chr(13) & _
										"</select>" & chr(13) & _
				"					</TD>" & chr(13) & _
				"				</TR>" & chr(13) & _
				"			</table>" & chr(13) & _
	...
	'> SOLUÇÃO
		x = x & "	<TR style='display:none;' id='TR_SOLUCAO_" & Cstr(intQtdeChamados) & "'>" & chr(13) & _
				...
				"							<TR>" & chr(13) & _
				"								<td>" & chr(13) & _
													"<textarea name='c_solucao_" & Cstr(intQtdeChamados) & "' id='c_solucao_" & Cstr(intQtdeChamados) & "' class='PLLe' rows='7' style='width:100%;margin-left:2pt;' onkeypress='limita_tamanho(this,MAX_TAM_DESCRICAO_CHAMADO_EM_PEDIDOS);' onblur='this.value=trim(this.value);calcula_tamanho_restante_solucao(" & chr(34) & Cstr(intQtdeChamados) & chr(34) & ");' onkeyup='calcula_tamanho_restante_solucao(" & chr(34) & Cstr(intQtdeChamados) & chr(34) & ");'></textarea>" & _
				"								</td>" & chr(13) & _
				"							</TR>" & chr(13) & _
				"						</table>" & chr(13) & _
				"					</TD>" & chr(13) & _
...
'	TOTAL GERAL
	if intQtdeChamados > 0 then
		x = x & "	<TR>" & chr(13) & _
				...
				"		<TD COLSPAN='11' class='MT'><p class='C'>TOTAL: &nbsp; " & cstr(qtde_chamado_aberto+qtde_chamado_em_andamento) & " chamado(s)</p></TD>" & chr(13) & _
				"	</TR>" & chr(13)
		end if
...
var cqo, cto, cs, cp, i, n, s, cmsg, cmsgna;
...
<input type="hidden" name="c_rel_origem" id="c_rel_origem" value="RELATORIO_CHAMADOS" />
<input type="hidden" name="blnHasDepto" id="blnHasDepto" value="<%=blnHasDepto%>" />
<input type="hidden" name="c_depto" id="c_depto" value="<%=c_depto%>" />
