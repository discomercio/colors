...
'	VERIFICA PERMISSÃO DE ACESSO DO USUÁRIO
	if Not operacao_permitida(OP_LJA_VENDAS_POR_BOLETO, s_lista_operacoes_permitidas) then 
		cn.Close
...
dim x, r, strSql, strResp, ha_default
	...
	strSql = "SELECT" & _
			...
			" FROM (" & _
					"SELECT DISTINCT analise_credito_usuario FROM t_PEDIDO WHERE (numero_loja = " & loja & ") AND (analise_credito <> " & COD_AN_CREDITO_ST_INICIAL & ") AND (analise_credito <> " & COD_AN_CREDITO_NAO_ANALISADO & ")" & _
				") tP" & _
...
<!--  ENTREGUE ENTRE  -->
	...
	<td class="MT" align="left" nowrap>
		...
		<input class="PLLc" maxlength="10" style="width:70px;" name="c_dt_entregue_inicio" id="c_dt_entregue_inicio"
			...
			onblur="if (!isDate(this)) {alert('Data inválida!'); this.focus();}" 
			onkeypress="if (digitou_enter(true)) fFILTRO.c_vendedor.focus(); filtra_data();"
			>
			</td></tr>
		</table>
		</td></tr>
