...
<%
'     ======================================================================
'	  R E L P E D I D O S A N T E R I O R E S P E S Q C L I E N T E . A S P
'     ======================================================================
'
'
...
'	PARÂMETROS DE PESQUISA
	dim rb_op, c_cnpj_cpf, c_nome_completo, c_nome_parcial, c_telefone, c_ddd, telefone_ddd
	...
	telefone_ddd = retorna_so_digitos(Trim(Request.Form("c_telefone")))
	c_ddd = mid(telefone_ddd,1,2)
	c_telefone = mid(telefone_ddd, 3)
...
dim r
  ...
  ' CABEÇALHO
	...
	select case rb_op
		...
		case "4"
		    if telefone_ddd <> "" then s = " ((ddd_res IN ('" & c_ddd & "', '0" & c_ddd & "') Or ddd_com IN ('" & c_ddd & "', '0" & c_ddd & "') Or ddd_cel IN ('" & c_ddd & "', '0" & c_ddd & "') Or ddd_com_2 IN ('" & c_ddd & "', '0" & c_ddd & "')) And (tel_res='" & c_telefone & "' Or tel_com='" & c_telefone & "' Or tel_com_2='" & c_telefone & "' Or tel_cel='" & c_telefone & "'))"
		case else
			s = ""
		end select
...
<br>
<center>
<form method="post" action="RelPedidosAnterioresPesqCliente.asp" id="fORD" name="fORD">
...
<form method="post" action="RelPedidosAnterioresExec.asp" id="fOP" name="fOP">
<%=MontaCampoFormSessionCtrlInfo(Session("SessionCtrlInfo"))%>
<input type="hidden" name="cliente_selecionado" id="cliente_selecionado" value="<%=cliente_unico_encontrado%>">
<input type="hidden" name="rb_op" id="rb_op" value="-1">
...
'	FECHA CONEXAO COM O BANCO DE DADOS
	...
	if cliente_unico_encontrado<>"" then
		Response.Redirect("RelPedidosAnterioresExec.asp?rb_op=-1&cliente_selecionado=" & cliente_unico_encontrado & "&" & MontaCampoQueryStringSessionCtrlInfo(Session("SessionCtrlInfo")))
		end if
%>
