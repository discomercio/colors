...
'	REVISADO P/ IE10
	...
	dim s, usuario, loja, pedido_selecionado,s_sql,numero_arclube, AutoSplitQty
	...
	AutoSplitQty = Trim(Request("AutoSplitQty"))

	pedido_selecionado = ucase(Trim(request("pedido_selecionado")))
	numero_arclube = Trim(Request("numero_arclube"))
...
'	=========================
	...
	if Not le_pedido(pedido_selecionado, r_pedido, msg_erro) then 
		...
		if Trim(r_pedido.loja) <> loja then 
			if PossuiAcessoLoja(usuario, r_pedido.loja) then 
				Response.Redirect("PedidoConsulta.asp?pedido_selecionado=" & pedido_selecionado & "&url_back=X" & "&" & MontaCampoQueryStringSessionCtrlInfo(Session("SessionCtrlInfo")))
			else
				Response.Redirect("aviso.asp?id=" & ERR_PEDIDO_ACESSO_NEGADO)
				end if
			end if
	...
	dim s_aux, s2, s3, s4, r_loja, r_cliente, s_cor, s_falta, v_pedido, v_pedido_detalhe_split
	dim v_disp
	...
	dim blnTemPagtoCartao
	blnTemPagtoCartao = False
	if alerta = "" then
		s = "SELECT" & _
				" Count(*) AS qtde" & _
			" FROM t_PAGTO_GW_PAG pag" & _
				" INNER JOIN t_PAGTO_GW_PAG_PAYMENT pay ON (pag.id = pay.id_pagto_gw_pag)" & _
			" WHERE" & _
				" (pedido LIKE '" & retorna_num_pedido_base(pedido_selecionado) & BD_CURINGA_TODOS & "')" & _
				" AND (ult_GlobalStatus IN " & _
						"(" & _
						"'" & BRASPAG_PAGADOR_CARTAO_GLOBAL_STATUS__AUTORIZADA & "'" & _
						"," & "'" & BRASPAG_PAGADOR_CARTAO_GLOBAL_STATUS__CAPTURADA & "'" & _
						"," & "'" & BRASPAG_PAGADOR_CARTAO_GLOBAL_STATUS__INDEFINIDA & "'" & _
						")" & _
					")"
		set rs = cn.Execute(s)
		if Not rs.Eof then
			if CLng(rs("qtde")) > 0 then blnTemPagtoCartao = True
			end if
		end if

	dim strJsScript
	strJsScript = ""
	if converte_numero(AutoSplitQty) > 1 then
		strJsScript = "<script language='javascript' type='text/javascript'>" & chr(13) & _
					"	$( function() {" & chr(13) & _
					"		exibeDialogModal('O pedido foi splitado automaticamente!', '');" & chr(13) & _
					"	});" & chr(13) & _
					"</script>" & chr(13)
		end if
...
' EXIBE_FAMILIA_PEDIDO
'
function sub_codigo_descricao_monta_itens_select(byval grupo_pai,byval codigo_pai, byval id_default)
dim s, x, r, strResp, ha_default
	...
	sub_codigo_descricao_monta_itens_select = strResp
end function
...
<script language="JavaScript" type="text/javascript">
	$(document).ready(function() {
		...
		$("#PainelBaseChaveAcessoNotaVenda").css('filter', 'alpha(opacity=30)');
		$("#PainelBaseChaveAcessoNotaRemessa").css('filter', 'alpha(opacity=30)');
		...
		function sizeFormMotivo() {
		    ...
		    $("#PainelBaseChaveAcessoNotaVenda").css("top", newTop);
		    $("#PainelChaveAcessoNotaVenda").css("top", newTop);
		    $("#PainelBaseChaveAcessoNotaRemessa").css("top", newTop);
		    $("#PainelChaveAcessoNotaRemessa").css("top", newTop);
		}
	    //Dynamically assign height
		...
		if ($(document).height() > $(window).height()) {
			...
			$("#PainelChaveAcessoNotaVenda").css('margin-top', '12%');
			$("#PainelChaveAcessoNotaRemessa").css('margin-top', '12%');
		}
		else {
			$("#FormMotivo").css('margin-top', '-175px');
			$("#PainelChaveAcessoNotaVenda").css('margin-top', '-175px');
			$("#PainelChaveAcessoNotaRemessa").css('margin-top', '-175px');
		}
});
...
function exibePainelChaveAcessoNotaVenda() {
    $("#PainelBaseChaveAcessoNotaVenda").css('display', 'block');
    $("#PainelChaveAcessoNotaVenda").css('display', 'block');
}
function exibePainelChaveAcessoNotaRemessa() {
    $("#PainelBaseChaveAcessoNotaRemessa").css('display', 'block');
    $("#PainelChaveAcessoNotaRemessa").css('display', 'block');
}
function fechaPainelChaveAcessoNotaVenda() {
    $("#PainelBaseChaveAcessoNotaVenda").css('display', 'none');
    $("#PainelChaveAcessoNotaVenda").css('display', 'none');
}
function fechaPainelChaveAcessoNotaRemessa() {
    $("#PainelBaseChaveAcessoNotaRemessa").css('display', 'none');
    $("#PainelChaveAcessoNotaRemessa").css('display', 'none');
}
function copiaChaveAcessoVenda() {
	var c;
	c = document.getElementById("c_chave_acesso_venda");
	c.select();
	c.createTextRange().execCommand("Copy");
}
function copiaChaveAcessoVendaComPrefixo() {
	var c;
	c = document.getElementById("c_chave_acesso_venda_com_prefixo");
	c.select();
	c.createTextRange().execCommand("Copy");
}
...
function exibeDialogModal(msgLinha1, msgLinha2)
{
	$('#dialog-message-p1').text(msgLinha1);
	$('#dialog-message-p2').text(msgLinha2);
...
function fPEDPagto(f) {
	...
	f.action = "../GwBpCs/P010PreRequisitos.asp";
	<% else %>
	f.action = "../GwBp/PagtoPreRequisitos.asp";
	<% end if %>
...
#PainelBaseChaveAcessoNotaVenda {
        position: absolute;
        width: 100%;
        height: 100%;
        top: 0;
...
#PainelBaseChaveAcessoNotaRemessa {
        position: absolute;
        width: 100%;
        height: 100%;
        top: 0;
...
#PainelChaveAcessoNotaRemessa {
	...
	.ui-widget-header {
		/* Caption do painel modal usado p/ exibir alertas */
		background: #f7ef00;
		border: solid 2px;
	}
</style>
...
<!--  E-MAIL DO CLIENTE  -->
<%  notPrint = ""
    if Trim(r_cliente.email) = "" then notPrint=" notPrint" %>
...
<table class="Q" style="width:649px;max-width: 649px" cellspacing="0">
	...
	<tr>
		<td class="MB" colspan="7" align="left"><p class="Rf">Observações II</p>
			<input name="c_obs2" id="c_obs2" class="PLLe" style="width:85px;margin-left:2pt;" 
				readonly tabindex=-1 value='<%=r_pedido.obs_2%>'>
		</td>
	</tr>
	<tr>
		<td class="MDB" align="left" nowrap width="10%"><p class="Rf">Parcelas</p>
...
<%  notPrint = ""
    if Trim(r_pedido.NFe_xPed) = "" then notPrint="class='notPrint'" %>
    <tr <%=notPrint%>>
...
<tr>
	...
	<td width="16.67%" class="MD" align="right"><span class="Cd"><%=s%></span></td>
	<% s = formata_moeda(vl_TotalFamiliaPago) %>
	<td width="16.67%" class="MD" align="right"><span class="Cd" style="color:<%
		...
		<td colspan="2" class="ME MD" style="width:<%=649-3-2*n_offset_tabela_devolucao%>px;" align="left">
			...
			<tr class="notPrint">
				<td class="MC" style="padding:0px;" align="left">
					...
					<tr>
					<td align="left">&nbsp;</td>
                    <% if operacao_permitida(OP_LJA_PRE_DEVOLUCAO_ESCREVER_MSG, s_lista_operacoes_permitidas) then %>
...
<% if operacao_permitida(OP_LJA_OCORRENCIAS_EM_PEDIDOS_LEITURA, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_OCORRENCIAS_EM_PEDIDOS_CADASTRAMENTO, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_OCORRENCIAS_EM_PEDIDOS_ESCREVER_MSG, s_lista_operacoes_permitidas) then %>
		...
		<td class="ME MD" style="width:<%=649-3-2*n_offset_tabela_ocorrencia%>px;" align="left">
			...
			<tr class="notPrint">
				<td class="MC" style="padding:0px;" align="left">
					...
					<tr>
					<td align="left">&nbsp;</td>
					<% if operacao_permitida(OP_LJA_OCORRENCIAS_EM_PEDIDOS_ESCREVER_MSG, s_lista_operacoes_permitidas) _
						Or _
						( _
							operacao_permitida(OP_LJA_OCORRENCIAS_EM_PEDIDOS_CADASTRAMENTO, s_lista_operacoes_permitidas) And _
							  ( _
								(Ucase(Trim("" & rs("usuario_cadastro")))= Ucase(usuario)) _
								OR _
								(Ucase(r_pedido.vendedor) = Ucase(usuario)) _
							  ) _
						) then %>
					<td align="center" class="ME MB" style="width:124px;padding:6px;">
...
%>
	<tr class="notPrint">
		<td colspan="3" style="padding:0px;" align="left">
			...
			<td align="left">&nbsp;</td>
			<% if operacao_permitida(OP_LJA_OCORRENCIAS_EM_PEDIDOS_CADASTRAMENTO, s_lista_operacoes_permitidas) And _
...
<% s = "SELECT t_PEDIDO_CHAMADO.usuario_cadastro AS chamado_usuario," & _
       ...
       " LEFT JOIN t_PEDIDO ON (t_PEDIDO.pedido = t_PEDIDO_CHAMADO.pedido)" & _
	...
	<tr>
		...
		<td class="ME MD" style="width:<%=649-3-2*n_offset_tabela_chamado%>px;" align="left">
			...
			<% if CInt(rs("finalizado_status"))=0 And _
                (operacao_permitida(OP_LJA_PEDIDO_CHAMADO_ESCREVER_MSG_QUALQUER_CHAMADO, s_lista_operacoes_permitidas) Or _
                operacao_permitida(OP_LJA_REL_PEDIDO_CHAMADO, s_lista_operacoes_permitidas) Or _
                blnIsUsuarioResponsavelDepto Or _
                blnIsUsuarioCadastroChamado) then %>
			<tr class="notPrint">
				<td class="MC" style="padding:0px;" align="left">
...
%>
	...
	<tr class="notPrint">
		<td colspan="4" style="padding:0px;" align="left">
			...
			<td align="left">&nbsp;</td>
			<% if operacao_permitida(OP_LJA_BLOCO_NOTAS_PEDIDO_CADASTRAMENTO, s_lista_operacoes_permitidas) then %>
			<td align="center" class="ME MB MD" style="width:45px;padding:2px;">
...
%>
	...
	<tr class="notPrint">
		<td colspan="3" style="padding:0px;" align="left">
			...
			<td align="left">&nbsp;</td>
			<% if operacao_permitida(OP_LJA_BLOCO_NOTAS_ITEM_DEVOLVIDO_CADASTRAMENTO, s_lista_operacoes_permitidas) then %>
			<td align="center" class="ME MB MD" style="width:45px;padding:2px;">
...
'	NA LOJA, SOMENTE OS DADOS DE PAGAMENTO POR CARTÃO PODEM SER CONSULTADOS
...
<% if operacao_permitida(OP_LJA_EDITA_PEDIDO, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_ITEM_DO_PEDIDO, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_RA, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_RT, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_PEDIDO_OBS1, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_PEDIDO_FORMA_PAGTO, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_ANALISE_CREDITO_PENDENTE_VENDAS, s_lista_operacoes_permitidas) Or _
	  operacao_permitida(OP_LJA_EDITA_PEDIDO_GARANTIA_INDICADOR, s_lista_operacoes_permitidas) then
	  blnExibeBotaoEditarPedido = True
	  end if
	  
	if IsPedidoCancelavelNaLoja(r_pedido.st_entrega, r_pedido.analise_credito) then blnExibeBotaoCancelarPedido = True
	...
	if blnTemPagtoCartao then blnExibeBotaoCancelarPedido = False
	
	if operacao_permitida(OP_LJA_CADASTRA_DEVOLUCAO_PRODUTO, s_lista_operacoes_permitidas) then
...
<tr>
	...
	<td align="center">
		<div name="dIMPRIME" id="dIMPRIME">
			<a name="bIMPRIME" id="bIMPRIME" href="javascript:fPEDImprime(fPED)" title="vai para a página de impressão do pedido em formulário contínuo">
	...
	<td align="right">
	<% if blnExibeBotaoEditarPedido then %>
...
<% if blnExibeBotaoDevolucao Or blnExibeBotaoPreDevolucao Or blnExibeBotaoPagamento then %>
<tr>
	<td align="left">
	...
	<td align="left">&nbsp;</td>
	<td align="right">
	<% if blnExibeBotaoPagamento then %>
		<div name='dPAGTO' id='dPAGTO'>
			<a name="bPAGTO" id="bPAGTO" href="javascript:fPEDPagto(fPED)" title="opções de pagamento">
				<img src="../botao/pagamento.gif" width="176" height="55" border="0">
			</a>
		</div>
	<% end if %>
	</td>
...
<!-- div PainelBaseChaveAcessoNotaVenda -->
    ...
    <table cellspacing="0" style="width:650px;">
        ...
        <tr>
            <td colspan="3" width="100%" align="center" style="padding:5px">
                <a name="bFECHAR" href="javascript:fechaPainelChaveAcessoNotaVenda();" title="voltar">
                <img src="../botao/voltar.gif" width="176" height="55" border="0"></a>
            </td>
        </tr>
    </table>
...
<!-- div PainelBaseChaveAcessoNotaRemessa -->
    ...
    <table cellspacing="0" style="width:650px;">
        ...
        <tr>
            <td colspan="3" width="100%" align="center" style="padding:5px">
                <a name="bFECHAR" href="javascript:fechaPainelChaveAcessoNotaRemessa();" title="voltar">
                <img src="../botao/voltar.gif" width="176" height="55" border="0"></a>
            </td>
        </tr>
    </table>
...
<div id="dialog-message" title="Atenção!">
  <p id="dialog-message-p1"></p>
  <p id="dialog-message-p2"></p>
</div>
