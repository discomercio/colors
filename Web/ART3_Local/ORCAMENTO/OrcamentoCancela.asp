...
'	~~~~~~~~~~~~~
	...
	rs.Open s, cn
	if rs.EOF then
		alerta="Orçamento " & orcamento_selecionado & " não está cadastrado."
	else
		s = Trim("" & rs("st_orcamento"))
		if s = ST_ORCAMENTO_CANCELADO then
			alerta="Orçamento " & orcamento_selecionado & " já foi cancelado em " & formata_data(rs("cancelado_data")) & "."
		elseif UCase(Trim("" & rs("orcamentista"))) <> Ucase(usuario) then
			alerta="Orçamento inválido para este orçamentista."
		elseif rs("st_orc_virou_pedido") = 1 then
			alerta="Não é possível cancelar orçamento que gerou pedido."
		else
		'	CANCELA O ORÇAMENTO
		'	===================
	...
	if Err=0 then 
		if alerta = "" then Response.Redirect("orcamento.asp?orcamento_selecionado=" & orcamento_selecionado & "&url_back=X")
	else
