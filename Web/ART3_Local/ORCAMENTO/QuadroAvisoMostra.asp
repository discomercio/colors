...
'	CONECTA COM O BANCO DE DADOS
	...
	if opcao_selecionada = "" then
		if Not recupera_avisos_nao_lidos(loja, usuario, vMsg) then 
		...
		if Not recupera_avisos(loja, usuario, vMsg) then 
			if opcao_alerta_se_nao_ha_aviso="" then
				Response.Redirect("resumo.asp")
			else
				alerta = "NÃO HÁ AVISOS."
...
<% if alerta = "" then %>
	...
	<table width="649" cellspacing="0">
		...
		<textarea name="mensagem" id="mensagem" type="textarea" class="QuadroAviso" readonly onkeypress="filtra_nome_identificador();"><%=vMsg(iMsg).mensagem%></textarea>
...
<tr>
	...
	<td align=<%=s%>><a href="Resumo.asp" title="cancela as alterações">
