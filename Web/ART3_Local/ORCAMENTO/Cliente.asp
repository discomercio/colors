	...
	<tr>
		<td nowrap>
			...
			<input type="radio" name="rb_op" id="rb_op" value="2" class="CBOX" onclick="fOP.c_cons.focus();"><span style="cursor:default" onclick="fOP.rb_op[1].click();fOP.c_cons.focus();">Consultar CNPJ/CPF</span>&nbsp;
				<input name="c_cons" id="c_cons" type="text" maxlength="18" size="20" onblur="if (!cnpj_cpf_ok(this.value)) {alert('CNPJ/CPF inválido!!');this.focus();} else this.value=cnpj_cpf_formata(this.value);" onclick="fOP.rb_op[1].click();" onkeypress="this.click(); if (digitou_enter(true) && tem_info(this.value) && cnpj_cpf_ok(this.value)) {this.value=cnpj_cpf_formata(this.value); if (fOPConcluir(fOP)) submit();} filtra_cnpj_cpf();"><br>
			</td>
		</tr>
	</table>
...
<table cellspacing="0">
<tr>
	<td align="center"><a href="Resumo.asp" title="volta para a página anterior">
