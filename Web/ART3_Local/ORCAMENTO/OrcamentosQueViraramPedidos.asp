...
<%
'     =================================================================
'	  O R C A M E N T O S Q U E V I R A R A M P E D I D O S . A S P
'     =================================================================
'
'
...
'	CRITÉRIO: ORÇAMENTOS QUE VIRARAM PEDIDOS
	s = "(t_ORCAMENTO.st_orc_virou_pedido=1)"
	if s_where <> "" then s_where = s_where & " AND"
	s_where = s_where & " (" & s & ")"

'	CRITÉRIO: ÚLTIMOS 60 DIAS
	s = "(t_PEDIDO.data >= " & bd_monta_data(Date-60) & ")"
	if s_where <> "" then s_where = s_where & " AND"
	s_where = s_where & " (" & s & ")"

'	CRITÉRIO: ORÇAMENTISTA (CADA ORÇAMENTISTA SÓ PODE CONSULTAR SEUS PRÓPRIOS ORÇAMENTOS)
	s = "(t_ORCAMENTO.orcamentista = '" & usuario & "')"
	if s_where <> "" then s_where = s_where & " AND"
	s_where = s_where & " (" & s & ")"
...
'	MONTA CLÁUSULA FROM
	s_from = " FROM t_ORCAMENTO INNER JOIN t_CLIENTE ON (t_ORCAMENTO.id_cliente=t_CLIENTE.id)" & _
  ...
  ' CABEÇALHO
	...
	cab = "	<TR style='background:azure'>" & chr(13) & _
		  "		<TD class='MDTE' style='width:60px' valign='bottom' NOWRAP><P class='R'>Pré-Pedido</P></TD>" & chr(13) & _
		  "		<TD class='MTD' style='width:50px' valign='bottom'><P class='R'>DT&nbsp;Orçam</P></TD>" & chr(13) & _
		  "		<TD class='MTD' style='width:60px' valign='bottom' NOWRAP><P class='R'>Pedido</P></TD>" & chr(13) & _
		  "		<TD class='MTD' style='width:50px' valign='bottom'><P class='R'>DT&nbsp;Ped</P></TD>" & chr(13) & _
		  ...
		  ' FECHA TABELA DA LOJA ANTERIOR
			if n_reg > 0 then 
				x = x & "	<TR style='background: #FFFFDD'>" & chr(13) & _
						"		<TD class='MTBE' colspan='5' NOWRAP><p class='Cd'>TOTAL:</p></td>" & chr(13) & _
	...
	'> Nº ORÇAMENTO
		x = x & "		<TD valign='top' class='MDTE'><P class='C'>&nbsp;<a href='javascript:fRELConcluir(" & _
				...
				")' title='clique para consultar o pedido'>" & Trim("" & r("orcamento")) & "</a></P></TD>" & chr(13)

	'> DATA
		s = formata_data(r("data_orcamento"))
		x = x & "		<TD valign='top' class='MTD'><P class='Cc'>" & s & "</P></TD>" & chr(13)

	'> Nº PEDIDO
		x = x & "		<TD valign='top' class='MTD'><P class='C'>&nbsp;<a href='javascript:fRELConcluir(" & _
				chr(34) & Trim("" & r("pedido")) & chr(34) & _
  ...
  ' MOSTRA TOTAL DA ÚLTIMA LOJA
	if n_reg <> 0 then 
		x = x & "	<TR style='background: #FFFFDD'>" & chr(13) & _
				"		<TD colspan='5' class='MTBE' NOWRAP><p class='Cd'>TOTAL:</p></td>" & chr(13) & _
				"		<TD class='MTB'><p class='Cd'>" & formata_moeda(vl_sub_total) & "</p></td>" & chr(13) & _
				"		<TD class='MTBD'><p class='C'>&nbsp;</p></td>" & chr(13) & _
				"	</TR>" & chr(13)
	...
	'>	TOTAL GERAL
		if qtde_lojas > 1 then
			x = x & _
				...
				"		<TD class='MTBE' colspan='5' NOWRAP><p class='Cd'>TOTAL GERAL:</p></td>" & chr(13) & _
  ...
  ' MOSTRA AVISO DE QUE NÃO HÁ DADOS!!
	if n_reg_total = 0 then
		x = cab_table & cab
		x = x & "	<TR>" & chr(13) & _
				"		<TD class='MT' colspan='7'><P class='ALERTA'>&nbsp;NENHUM PRÉ-PEDIDO ENCONTRADO&nbsp;</P></TD>" & chr(13) & _
				"	</TR>" & chr(13)
		end if
...
<tr>
	<td align="right" valign="bottom"><span class="PEDIDO">Pré-Pedidos Que Viraram Pedidos</span>
	<br><span class="Rc">
